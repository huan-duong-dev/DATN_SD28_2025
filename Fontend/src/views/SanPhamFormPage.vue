...
const searchQueries = ref<{ [key: string]: string }>({})
const showSearchableDropdowns = ref<{ [key: string]: boolean }>({})
const filteredOptions = ref<{ [key: string]: any[] }>({})
...
type Variant = {
  ...
  giaNhap: number
  ghiChu: string
  trangThai?: number // Trạng thái biến thể (0: inactive, 1: active)
...
const canSaveUpdate = computed(
  () => !!form.value.tenSanPham && !!form.value.idHang && !!form.value.idDanhMuc,
)
...
function addVariant() {
  ...
  if (!form.value.tenSanPham) {
    toastRef.value?.warning(
      'Thiếu thông tin',
      'Vui lòng nhập tên sản phẩm trước khi thêm phiên bản',
    )
    return
  }
  ...
  if (
    variantForm.value.selectedRams.length === 0 ||
    variantForm.value.selectedRoms.length === 0 ||
    variantForm.value.selectedMauSacs.length === 0
  ) {
  ...
  for (const ramId of variantForm.value.selectedRams) {
    for (const romId of variantForm.value.selectedRoms) {
      for (const mauSacId of variantForm.value.selectedMauSacs) {
        // Kiểm tra xem combo này đã tồn tại chưa
        const isDuplicate = variants.value.some(
          (v) => v.idRam === ramId && v.idRom === romId && v.idMauSac === mauSacId,
        )

        if (isDuplicate) {
          duplicateCount++
          console.log(`Bỏ qua combo trùng: RAM ${ramId}, ROM ${romId}, Màu ${mauSacId}`)
          continue // Bỏ qua combo đã tồn tại
        }
        ...
        variants.value.push({
          ...
          trangThai: 1, // Mặc định trạng thái = 1 (active) cho biến thể mới
  ...
  if (addedCount > 0 && duplicateCount > 0) {
    toastRef.value?.success(
      'Thành công',
      `Đã thêm ${addedCount} phiên bản mới. ${duplicateCount} phiên bản trùng lặp đã được bỏ qua.`,
    )
  } else if (addedCount > 0) {
    toastRef.value?.success('Thành công', `Đã thêm ${addedCount} phiên bản mới`)
  } else if (duplicateCount > 0) {
    toastRef.value?.warning(
      'Thông báo',
      `Tất cả ${duplicateCount} phiên bản đã tồn tại. Không có phiên bản mới nào được thêm.`,
    )
  }
...
function getSelectedRamsText() {
  ...
  return variantForm.value.selectedRams
    .map((id) => getAttributeName(id, rams.value, 'tenRam'))
    .join(', ')
}

function getSelectedRomsText() {
  if (variantForm.value.selectedRoms.length === 0) return 'Chọn ROM'
  return variantForm.value.selectedRoms
    .map((id) => getAttributeName(id, roms.value, 'dungLuong'))
    .join(', ')
}

function getSelectedMauSacsText() {
  if (variantForm.value.selectedMauSacs.length === 0) return 'Chọn Màu sắc'
  return variantForm.value.selectedMauSacs
    .map((id) => getAttributeName(id, mauSacs.value, 'tenMau'))
    .join(', ')
}
...
function generateCode(prefix: string, name?: string): string {
  ...
  if (name) {
    // Tạo mã từ tên: lấy chữ cái đầu của mỗi từ và chuyển thành uppercase
...
function openAddModal(type: string) {
  ...
  // Khởi tạo các trường cần thiết cho từng loại (không bao gồm mã - sẽ tự động tạo)
  const fieldConfigs: Record<string, string[]> = {
    'danh-muc': ['tenDanhMuc'],
    'he-dieu-hanh': ['tenHeDieuHanh', 'moTa'],
    'man-hinh': ['kichThuoc', 'congNghe', 'doPhanGiai', 'tanSoQuet', 'kieuManHinh', 'moTa'],
    hang: ['ten', 'xuatXu', 'moTa'],
    'camera-truoc': ['thongSo', 'moTa'],
    'camera-sau': ['thongSo', 'moTa'],
    pin: ['dungLuongPin', 'congNgheSac', 'moTa'],
    chip: ['tenChip', 'moTa'],
    cpu: ['tenCpu', 'moTa'],
    gpu: ['tenGpu', 'moTa'],
    ram: ['tenRam', 'moTa'],
  ...
  try {
    ...
    switch (modalType.value) {
      case 'danh-muc':
        ...
        payload = {
          maDanhMuc: generateCode('DM', modalInputs.value.tenDanhMuc),
      ...
      case 'he-dieu-hanh':
        ...
        payload = {
          maHeDieuHanh: generateCode('HDH', modalInputs.value.tenHeDieuHanh),
      ...
      case 'man-hinh':
        ...
        payload = {
          maManHinh: generateCode('MH', modalInputs.value.kichThuoc),
      ...
      case 'camera-truoc':
        ...
        payload = {
          maCamera: generateCode('CT', modalInputs.value.thongSo),
      ...
      case 'camera-sau':
        ...
        payload = {
          maCamera: generateCode('CS', modalInputs.value.thongSo),
          thongSo: modalInputs.value.thongSo?.trim(),
          moTa: modalInputs.value.moTa?.trim(),
          trangThai: 1,
        }
      ...
      case 'pin':
        endpoint = '/api/pin'
        payload = {
          maPin: generateCode('PIN', modalInputs.value.dungLuongPin),
      ...
      case 'chip':
        endpoint = '/api/chip'
        payload = {
          maChip: generateCode('CHIP', modalInputs.value.tenChip),
      ...
      case 'cpu':
        endpoint = '/api/cpu'
        payload = {
          maCpu: generateCode('CPU', modalInputs.value.tenCpu),
      ...
      case 'gpu':
        endpoint = '/api/gpu'
        payload = {
          maGpu: generateCode('GPU', modalInputs.value.tenGpu),
      ...
      case 'ram':
        endpoint = '/api/ram'
        payload = {
          maRam: generateCode('RAM', modalInputs.value.tenRam),
      ...
      case 'rom':
        endpoint = '/api/rom'
        payload = {
          maRom: generateCode('ROM', modalInputs.value.dungLuong),
      ...
      case 'mau-sac':
        ...
        payload = {
          maMau: generateCode('MS', modalInputs.value.tenMau),
...
function getRequiredFields(type: string): string[] {
  const requiredFields: Record<string, string[]> = {
    'danh-muc': ['tenDanhMuc'],
    'he-dieu-hanh': ['tenHeDieuHanh'],
    'man-hinh': ['kichThuoc', 'congNghe'],
    hang: ['ten'],
    'camera-truoc': ['thongSo'],
    'camera-sau': ['thongSo'],
    pin: ['dungLuongPin'],
    chip: ['tenChip'],
    cpu: ['tenCpu'],
...
async function performDeleteVariant(variantIndex: number, variant: Variant, originalIndex: number) {
  // Lưu màu sắc của phiên bản trước khi xóa
  ...
  // Kiểm tra xem còn phiên bản nào sử dụng màu sắc này không
  if (colorId) {
    const remainingVariantsWithColor = variants.value.filter((v) => v.idMauSac === colorId)
    // Nếu không còn phiên bản nào sử dụng màu này, xóa hình ảnh của màu đó
    if (remainingVariantsWithColor.length === 0) {
      delete colorImages.value[colorId]
      // Cũng xóa khỏi variantImages nếu có
      if (variantImages.value[originalIndex]) {
        delete variantImages.value[originalIndex]
      }
    }
  }
    ...
    if (isCreate.value) {
      ...
      const payload = {
        ...
        variants: variants.value.map((v) => ({
          ...
          ghiChu: v.ghiChu || '',
          trangThai: v.trangThai !== undefined ? v.trangThai : 1, // Mặc định active cho biến thể mới
          imeis: v.imeis || [],
      ...
      for (let i = 0; i < payload.variants.length; i++) {
        ...
        if (!variant.idRam || !variant.idRom || !variant.idMauSac) {
          toastRef.value?.warning(
            'Thiếu thông tin',
            `Phiên bản ${i + 1} thiếu thông tin RAM, ROM hoặc màu sắc`,
          )
          return
        }
      }
      ...
      const payload = {
        ...
        idPin: form.value.idPin,
        variants: variants.value.map((v) => ({
          id: v.id, // ID của biến thể (nếu đã tồn tại)
          idRam: v.idRam,
          idRom: v.idRom,
          idMauSac: v.idMauSac,
          soLuong: v.imeis && v.imeis.length > 0 ? v.imeis.length : 0,
          ...
          giaNhap: Number(v.giaNhap) || 0,
          ghiChu: v.ghiChu || '',
          trangThai: v.trangThai !== undefined ? v.trangThai : 1, // Giữ nguyên trạng thái cũ hoặc active cho mới
          imeis: v.imeis || [],
          imageUrls: colorImages.value[v.idMauSac] || [],
        })),
      }
...
async function loadLookups() {
  try {
    ...
    if (dmRes.status === 'fulfilled') {
      danhMucs.value = dmRes.value.data.sort((a, b) => (b.id || 0) - (a.id || 0))
    }
    if (hRes.status === 'fulfilled') {
      hangs.value = hRes.value.data.sort((a, b) => (b.id || 0) - (a.id || 0))
      console.log('Hangs loaded:', hangs.value)
    } else {
      console.error('Hang request failed:', hRes.reason)
    }
    if (ramRes.status === 'fulfilled')
      rams.value = ramRes.value.data.sort((a, b) => (b.id || 0) - (a.id || 0))
    if (romRes.status === 'fulfilled')
      roms.value = romRes.value.data.sort((a, b) => (b.id || 0) - (a.id || 0))
    if (mauRes.status === 'fulfilled') {
      mauSacs.value = mauRes.value.data.sort((a, b) => (b.id || 0) - (a.id || 0))
      console.log('Loaded mauSacs:', mauSacs.value)
    }
    if (hdhRes.status === 'fulfilled')
      heDieuHanhs.value = hdhRes.value.data.sort((a, b) => (b.id || 0) - (a.id || 0))
    if (mhRes.status === 'fulfilled')
      manHinhs.value = mhRes.value.data.sort((a, b) => (b.id || 0) - (a.id || 0))
    if (ctRes.status === 'fulfilled') {
      cameraTruocs.value = ctRes.value.data.sort((a, b) => (b.id || 0) - (a.id || 0))
    }
    if (csRes.status === 'fulfilled') {
      cameraSaus.value = csRes.value.data.sort((a, b) => (b.id || 0) - (a.id || 0))
    }
    if (pinRes.status === 'fulfilled') {
      pins.value = pinRes.value.data.sort((a, b) => (b.id || 0) - (a.id || 0))
    ...
    if (chipRes.status === 'fulfilled')
      chips.value = chipRes.value.data.sort((a, b) => (b.id || 0) - (a.id || 0))
    if (cpuRes.status === 'fulfilled')
      cpus.value = cpuRes.value.data.sort((a, b) => (b.id || 0) - (a.id || 0))
    if (gpuRes.status === 'fulfilled')
      gpus.value = gpuRes.value.data.sort((a, b) => (b.id || 0) - (a.id || 0))
    if (simRes.status === 'fulfilled')
      sims.value = simRes.value.data.sort((a, b) => (b.id || 0) - (a.id || 0))
  ...
  try {
    ...
    if (productData.variants && productData.variants.length > 0) {
      variants.value = productData.variants.map((variant: any, index: number) => ({
        ...
        trangThai: variant.trangThai !== undefined ? variant.trangThai : 1, // Lưu trạng thái từ backend
        tenSanPham: productData.tenSanPham || '',
        idHang: productData.idHang || null,
        idDanhMuc: productData.idDanhMuc || null,
        moTa: productData.moTa || '',
      ...
      // Load selected values from ALL existing variants into variantForm
      if (variants.value.length > 0) {
        // Thu thập tất cả các giá trị đang được dùng bởi các variants
        const usedRams = new Set<number>()
        const usedRoms = new Set<number>()
        const usedMauSacs = new Set<number>()

        variants.value.forEach((v) => {
          if (v.idRam) usedRams.add(v.idRam)
          if (v.idRom) usedRoms.add(v.idRom)
          if (v.idMauSac) usedMauSacs.add(v.idMauSac)
        })

        // Prefill dropdown với TẤT CẢ các giá trị đang được dùng
        variantForm.value.selectedRams = Array.from(usedRams)
        variantForm.value.selectedRoms = Array.from(usedRoms)
        variantForm.value.selectedMauSacs = Array.from(usedMauSacs)

        console.log('DEBUG - Prefilled from existing variants:', {
          rams: variantForm.value.selectedRams,
          roms: variantForm.value.selectedRoms,
          mauSacs: variantForm.value.selectedMauSacs,
        })
      }

      // Load images into colorImages
      console.log('DEBUG - Loading colorImages from variants:', variants.value)
      variants.value.forEach((variant) => {
        if (variant.idMauSac && variant.imageUrls && variant.imageUrls.length > 0) {
          colorImages.value[variant.idMauSac] = [...variant.imageUrls]
          console.log(
            `DEBUG - Loaded ${variant.imageUrls.length} images for color ${variant.idMauSac}`,
          )
        }
      })
      console.log('DEBUG - Final colorImages:', colorImages.value)
    }
  } catch (error) {
...
function handleClickOutside(event: Event) {
  ...
  if (
    ...
    Object.keys(showSearchableDropdowns.value).forEach((key) => {
      showSearchableDropdowns.value[key] = false
    })
...
function filterOptions(
  ...
  let filteredResult = options

  if (searchQuery && searchQuery.trim() !== '') {
    const query = searchQuery.toLowerCase().trim()
    filteredResult = options.filter((option) => {
      const displayValue = (option[displayField] || '').toLowerCase()
      const codeValue = codeField ? (option[codeField] || '').toLowerCase() : ''
      return displayValue.includes(query) || codeValue.includes(query)
    })
  }

  // Sắp xếp kết quả theo id hoặc ngayTao giảm dần (mới nhất lên đầu)
  return filteredResult.sort((a, b) => {
    // Ưu tiên sắp xếp theo ngayTao nếu có
    if (a.ngayTao && b.ngayTao) {
      return new Date(b.ngayTao).getTime() - new Date(a.ngayTao).getTime()
    }
    // Nếu không có ngayTao, sắp xếp theo id giảm dần
    return (b.id || 0) - (a.id || 0)
  })
}

function onSearchInput(key: string, options: any[], displayField: string, codeField?: string) {
  const query = searchQueries.value[key] || ''
  filteredOptions.value[key] = filterOptions(key, options, query, displayField, codeField)
}
...
function toggleSearchableDropdown(
  ...
  Object.keys(showSearchableDropdowns.value).forEach((k) => {
    if (k !== key) {
      showSearchableDropdowns.value[k] = false
    }
  })

  showSearchableDropdowns.value[key] = !showSearchableDropdowns.value[key]

  if (showSearchableDropdowns.value[key]) {
    // Set this as active dropdown
    activeDropdownKey.value = key
    // Sắp xếp options theo id giảm dần (mới nhất lên đầu)
    ...
    filteredOptions.value[key] = sortedOptions
    // Reset search query
    ...
    delete filteredOptions.value[key]
  }
}

function selectOption(key: string, value: any, modelKey: string) {
  form.value[modelKey as keyof typeof form.value] = value
  showSearchableDropdowns.value[key] = false
  searchQueries.value[key] = ''
  // Clear active dropdown
  if (activeDropdownKey.value === key) {
...
function applyPriceToAll(groupKey: string) {
  ...
  if (group && group.priceInput) {
    ...
    if (!isNaN(price)) {
      ...
      toastRef.value?.error('Lỗi nhập liệu', 'Vui lòng nhập giá trị hợp lệ')
    }
  } else {
    toastRef.value?.warning('Thiếu thông tin', 'Vui lòng nhập giá trị')
  }
}
...
function deleteVariantGroup(groupKey: string) {
  const group = groupedVariants.value.find((g) => g.key === groupKey)
  if (group) {
    // Lưu danh sách màu sắc của các phiên bản sẽ bị xóa
    const colorIdsToCheck = new Set<number>()
    group.variants.forEach((v) => {
      if (v.idMauSac) {
        colorIdsToCheck.add(v.idMauSac)
      }
    })
    ...
    indicesToRemove.forEach((index) => {
      ...
      // Xóa variantImages tương ứng
      if (variantImages.value[index]) {
        delete variantImages.value[index]
      }
    })

    // Kiểm tra và xóa hình ảnh của các màu sắc không còn được sử dụng
    colorIdsToCheck.forEach((colorId) => {
      const remainingVariantsWithColor = variants.value.filter((v) => v.idMauSac === colorId)
      if (remainingVariantsWithColor.length === 0) {
        delete colorImages.value[colorId]
      }
    ...
    reader.onload = (e) => {
      try {
        ...
        if (imeiList.length > 0) {
          ...
          toastRef.value?.warning(
            'Cảnh báo',
            'Không tìm thấy IMEI hợp lệ trong file Excel. Vui lòng kiểm tra định dạng file.',
          )
          target.value = ''
        }
      } catch (error) {
...
async function startQRScanner() {
  try {
    ...
    if (!isSecure) {
      toastRef.value?.error(
        'Lỗi bảo mật',
        'Camera chỉ có thể truy cập qua HTTPS hoặc localhost. Vui lòng:\n' +
          '1. Sử dụng HTTPS để truy cập trang web\n' +
          '2. Hoặc truy cập qua localhost/127.0.0.1\n' +
          '3. Hoặc nhập IMEI thủ công vào ô nhập liệu',
      )
      return
    }
    ...
    const video = document.getElementById('imei-qr-scanner-video') as HTMLVideoElement
    if (!video) {
    ...
    qrScanner.value = new QrScanner(
      ...
      async (result) => {
        ...
        // Không đóng scanner, cho phép quét tiếp
      },
      {
        onDecodeError: (error) => {
          // Ignore decode errors during scanning
        },
        preferredCamera: 'environment', // Ưu tiên camera sau
        maxScansPerSecond: 5,
    ...
    if (error.name === 'NotAllowedError' || error.message?.includes('permission')) {
      errorMessage =
        'Bị từ chối quyền truy cập camera. Vui lòng:\n' +
        '1. Cho phép quyền camera trong cài đặt trình duyệt\n' +
        '2. Làm mới trang và thử lại\n' +
        '3. Hoặc nhập IMEI thủ công'
    } else if (error.name === 'NotFoundError' || error.message?.includes('camera')) {
      errorMessage = 'Không tìm thấy camera trên thiết bị. Vui lòng kiểm tra camera và thử lại.'
    } else if (error.name === 'NotReadableError' || error.message?.includes('in use')) {
      errorMessage =
        'Camera đang được sử dụng bởi ứng dụng khác. Vui lòng đóng ứng dụng khác và thử lại.'
    } else if (error.message?.includes('https') || error.message?.includes('secure')) {
      errorMessage =
        'Camera chỉ có thể truy cập qua HTTPS hoặc localhost. Vui lòng sử dụng HTTPS hoặc nhập IMEI thủ công.'
...
async function handleQRScanned(qrData: string) {
  try {
    ...
    if (
      ...
      toastRef.value?.warning(
        'Cảnh báo',
        'QR code này là một liên kết, không phải IMEI. Vui lòng quét mã IMEI của sản phẩm.',
      )
      return
    }

    // Kiểm tra nếu không phải số hoặc quá ngắn/dài
    const imeiPattern = /^[0-9]{10,20}$/
    if (!imeiPattern.test(trimmedData)) {
      toastRef.value?.warning(
        'Cảnh báo',
        'QR code không hợp lệ. IMEI phải là dãy số từ 10-20 chữ số.',
      )
      return
    }

    // Thêm IMEI vào input (thêm vào cuối, mỗi IMEI trên một dòng)
    if (imeiInput.value.trim()) {
      // Kiểm tra xem IMEI đã tồn tại chưa
      const existingImeis = imeiInput.value
        .split('\n')
        .map((line) => line.trim())
        .filter((line) => line)
      if (existingImeis.includes(trimmedData)) {
        toastRef.value?.warning('Cảnh báo', 'IMEI này đã được thêm vào danh sách.')
        return
      }
      imeiInput.value += '\n' + trimmedData
    } else {
      imeiInput.value = trimmedData
    }

    toastRef.value?.success('Thành công', `Đã thêm IMEI: ${trimmedData}`)
  } catch (error) {
    console.error('Error handling QR scan:', error)
    toastRef.value?.error('Lỗi', 'Có lỗi xảy ra khi xử lý QR code')
  }
}
...
async function saveImei() {
  ...
  if (currentVariantIndex.value !== null) {
    ...
    if (duplicateImeis.length > 0) {
      toastRef.value?.warning(
        'Cảnh báo',
        `Có ${duplicateImeis.length} IMEI trùng lặp trong danh sách đã được loại bỏ`,
      )
    }
    ...
    if (invalidImeis.length > 0) {
      toastRef.value?.warning(
        'Cảnh báo',
        `Có ${invalidImeis.length} IMEI không hợp lệ đã được bỏ qua`,
      )
    }
...
function continueWithExcelImport(
  imeiList: string[],
  dbDuplicates: string[],
  variantIndex: number,
...
function isDuplicateImei(imei: string): boolean {
  ...
  const currentVariant = variants.value[currentVariantIndex.value]
  if (!currentVariant) return false

  // Kiểm tra trùng lặp với IMEI đã lưu trong biến thể hiện tại
  const savedImeis = currentVariant.imeis || []
  if (savedImeis.includes(imei)) return true

  // Kiểm tra trùng lặp với IMEI ở CÁC BIẾN THỂ KHÁC
  for (let i = 0; i < variants.value.length; i++) {
    if (i === currentVariantIndex.value) continue // Bỏ qua biến thể hiện tại
    const otherVariant = variants.value[i]
    if (otherVariant && otherVariant.imeis && otherVariant.imeis.includes(imei)) {
      return true // IMEI đã tồn tại ở biến thể khác
    }
  }

  // Kiểm tra trùng lặp với IMEI đang nhập khác trong cùng biến thể
  const inputImeis = getInputImeis()
...
function loadDraftData() {
  ...
  try {
    ...
    if (saved) {
      ...
      // Chỉ load variants nếu có ít nhất 1 variant hợp lệ
      if (
        draftData.variants &&
        Array.isArray(draftData.variants) &&
        draftData.variants.length > 0
      ) {
        // Kiểm tra xem variants có hợp lệ không (có idRam, idRom, idMauSac)
        const validVariants = draftData.variants.filter(
          (v: any) => v && v.idRam && v.idRom && v.idMauSac,
        )
        if (validVariants.length > 0) {
          variants.value = validVariants
          // Update quantities based on IMEI count
          updateAllVariantQuantities()
        } else {
          // Nếu không có variant hợp lệ, xóa draft và reset variants
          localStorage.removeItem('sanpham-draft')
          variants.value = []
        }
      } else {
        // Nếu không có variants trong draft, đảm bảo variants rỗng
        variants.value = []
      }
...
<template>
  <div class="page">
    ...
    <div class="form-card">
      <div class="form-columns">
        <div class="form-column">
          ...
          <div
            class="form-group"
            :style="{ zIndex: activeDropdownKey === 'heDieuHanh' ? 10003 : 1 }"
          >
            <label>Hệ điều hành</label>
            <div class="select-with-add">
              <div
                class="searchable-select"
                :style="{ zIndex: activeDropdownKey === 'heDieuHanh' ? 10001 : 10 }"
              >
                <div
                ...
                >
                  <span class="select-text">{{
                    getDisplayText('heDieuHanh', heDieuHanhs, form.idHeDieuHanh, 'tenHeDieuHanh') ||
                    'Chọn hệ điều hành'
                  }}</span>
                  <span class="select-arrow">▼</span>
                </div>
                <div
                  v-if="showSearchableDropdowns['heDieuHanh']"
                  class="searchable-select-dropdown"
                  :style="{ zIndex: activeDropdownKey === 'heDieuHanh' ? 10002 : 10000 }"
                >
                  <div class="search-input-container">
                    <FontAwesomeIcon icon="search" class="search-icon" />
                    <input
                      v-model="searchQueries['heDieuHanh']"
                      @input="
                        onSearchInput('heDieuHanh', heDieuHanhs, 'tenHeDieuHanh', 'maHeDieuHanh')
                      "
                      type="text"
                  ...
                  <div class="select-options">
                    <div
                      v-for="hdh in filteredOptions['heDieuHanh'] !== undefined
                        ? filteredOptions['heDieuHanh']
                        : heDieuHanhs"
                      :key="hdh.id"
                      @click="selectOption('heDieuHanh', hdh.id, 'idHeDieuHanh')"
                      class="select-option"
                      :class="{ selected: form.idHeDieuHanh === hdh.id }"
                    >
                      <span class="option-code" v-if="hdh.maHeDieuHanh"
                        >{{ hdh.maHeDieuHanh }} -
                      </span>
                      <span class="option-name">{{ hdh.tenHeDieuHanh }}</span>
                    </div>
                    <div
                      v-if="
                        filteredOptions['heDieuHanh'] !== undefined &&
                        filteredOptions['heDieuHanh'].length === 0 &&
                        searchQueries['heDieuHanh']
                      "
                      class="no-results"
                    >
          ...
          <div
            class="form-group"
            :style="{ zIndex: activeDropdownKey === 'cameraTruoc' ? 10003 : 1 }"
          >
            <label>Camera trước</label>
            <div class="select-with-add">
              <div
                class="searchable-select"
                :style="{ zIndex: activeDropdownKey === 'cameraTruoc' ? 10001 : 10 }"
              >
                <div
                  class="searchable-select-input"
                  @click="
                    toggleSearchableDropdown('cameraTruoc', cameraTruocs, 'thongSo', 'maCamera')
                  "
                >
                  <span class="select-text">{{
                    getDisplayText('cameraTruoc', cameraTruocs, form.idCameraTruoc, 'thongSo') ||
                    'Chọn camera trước'
                  }}</span>
                  <span class="select-arrow">▼</span>
                </div>
                <div
                  v-if="showSearchableDropdowns['cameraTruoc']"
                  class="searchable-select-dropdown"
                  :style="{ zIndex: activeDropdownKey === 'cameraTruoc' ? 10002 : 10000 }"
                >
                  <div class="search-input-container">
                    <FontAwesomeIcon icon="search" class="search-icon" />
                    <input
                      v-model="searchQueries['cameraTruoc']"
                      @input="onSearchInput('cameraTruoc', cameraTruocs, 'thongSo', 'maCamera')"
                      type="text"
                      placeholder="Tìm theo mã, thông số..."
                      class="search-input"
                      @click.stop
                    />
                  </div>
                  <div class="select-options">
                    <div
                      v-for="ct in filteredOptions['cameraTruoc'] !== undefined
                        ? filteredOptions['cameraTruoc']
                        : cameraTruocs"
                      :key="ct.id"
                      @click="selectOption('cameraTruoc', ct.id, 'idCameraTruoc')"
                      class="select-option"
                      :class="{ selected: form.idCameraTruoc === ct.id }"
                    >
                      <span class="option-code" v-if="ct.maCamera">{{ ct.maCamera }} - </span>
                      <span class="option-name">{{ ct.thongSo }}</span>
                    </div>
                    <div
                      v-if="
                        filteredOptions['cameraTruoc'] !== undefined &&
                        filteredOptions['cameraTruoc'].length === 0 &&
                        searchQueries['cameraTruoc']
                      "
                      class="no-results"
                    >
                      Không tìm thấy kết quả
          ...
          <div class="form-group" :style="{ zIndex: activeDropdownKey === 'pin' ? 10003 : 1 }">
            <label>Pin</label>
            <div class="select-with-add">
              <div
                class="searchable-select"
                :style="{ zIndex: activeDropdownKey === 'pin' ? 10001 : 10 }"
              >
                <div
                  class="searchable-select-input"
                  @click="toggleSearchableDropdown('pin', pins, 'dungLuongPin', 'maPin')"
                >
                  <span class="select-text">{{
                    getDisplayText('pin', pins, form.idPin, 'dungLuongPin') || 'Chọn pin'
                  }}</span>
                  <span class="select-arrow">▼</span>
                </div>
                <div
                  v-if="showSearchableDropdowns['pin']"
                  class="searchable-select-dropdown"
                  :style="{ zIndex: activeDropdownKey === 'pin' ? 10002 : 10000 }"
                >
                  <div class="search-input-container">
                    <FontAwesomeIcon icon="search" class="search-icon" />
                    <input
                      v-model="searchQueries['pin']"
                      @input="onSearchInput('pin', pins, 'dungLuongPin', 'maPin')"
                      type="text"
                      placeholder="Tìm theo mã, dung lượng..."
                      class="search-input"
                      @click.stop
                    />
                  </div>
                  <div class="select-options">
                    <div
                      v-for="p in filteredOptions['pin'] !== undefined
                        ? filteredOptions['pin']
                        : pins"
                      :key="p.id"
                      @click="selectOption('pin', p.id, 'idPin')"
                      class="select-option"
                      :class="{ selected: form.idPin === p.id }"
                    >
                      <span class="option-code" v-if="p.maPin">{{ p.maPin }} - </span>
                      <span class="option-name">{{ p.dungLuongPin }}</span>
                    </div>
                    <div
                      v-if="
                        filteredOptions['pin'] !== undefined &&
                        filteredOptions['pin'].length === 0 &&
                        searchQueries['pin']
                      "
                      class="no-results"
                    >
        ...
        <div class="form-column">
          <div class="form-group" :style="{ zIndex: activeDropdownKey === 'danhMuc' ? 10003 : 1 }">
            <label>Danh mục</label>
            <div class="select-with-add">
              <div
                class="searchable-select"
                :style="{ zIndex: activeDropdownKey === 'danhMuc' ? 10001 : 10 }"
              >
                <div
                  class="searchable-select-input"
                  @click="toggleSearchableDropdown('danhMuc', danhMucs, 'tenDanhMuc', 'maDanhMuc')"
                >
                  <span class="select-text">{{
                    getDisplayText('danhMuc', danhMucs, form.idDanhMuc, 'tenDanhMuc') ||
                    'Chọn danh mục'
                  }}</span>
                  <span class="select-arrow">▼</span>
                </div>
                <div
                  v-if="showSearchableDropdowns['danhMuc']"
                  class="searchable-select-dropdown"
                  :style="{ zIndex: activeDropdownKey === 'danhMuc' ? 10002 : 10000 }"
                >
                  <div class="search-input-container">
                    <FontAwesomeIcon icon="search" class="search-icon" />
                    <input
                      v-model="searchQueries['danhMuc']"
                      @input="onSearchInput('danhMuc', danhMucs, 'tenDanhMuc', 'maDanhMuc')"
                      type="text"
                      placeholder="Tìm theo mã, tên..."
                      class="search-input"
                      @click.stop
                  ...
                  <div class="select-options">
                    <div
                      v-for="dm in filteredOptions['danhMuc'] || danhMucs"
                      :key="dm.id"
                      @click="selectOption('danhMuc', dm.id, 'idDanhMuc')"
                      class="select-option"
                      :class="{ selected: form.idDanhMuc === dm.id }"
                    >
                      <span class="option-code" v-if="dm.maDanhMuc">{{ dm.maDanhMuc }} - </span>
                      <span class="option-name">{{ dm.tenDanhMuc }}</span>
                    </div>
                    <div
                      v-if="
                        (filteredOptions['danhMuc'] || []).length === 0 && searchQueries['danhMuc']
                      "
                      class="no-results"
                    >
                      Không tìm thấy kết quả
          ...
          <div class="form-group" :style="{ zIndex: activeDropdownKey === 'manHinh' ? 10003 : 1 }">
            <label>Màn hình</label>
            <div class="select-with-add">
              <div
                class="searchable-select"
                :style="{ zIndex: activeDropdownKey === 'manHinh' ? 10001 : 10 }"
              >
                <div
                  class="searchable-select-input"
                  @click="toggleSearchableDropdown('manHinh', manHinhs, 'kichThuoc', 'maManHinh')"
                >
                  <span class="select-text">{{
                    form.idManHinh
                      ? manHinhs.find((mh) => mh.id === form.idManHinh)?.kichThuoc +
                        (manHinhs.find((mh) => mh.id === form.idManHinh)?.doPhanGiai
                          ? ' - ' + manHinhs.find((mh) => mh.id === form.idManHinh)?.doPhanGiai
                          : '')
                      : 'Chọn màn hình'
                  }}</span>
                ...
                <div
                  v-if="showSearchableDropdowns['manHinh']"
                  class="searchable-select-dropdown"
                  :style="{ zIndex: activeDropdownKey === 'manHinh' ? 10002 : 10000 }"
                >
                  <div class="search-input-container">
                    <FontAwesomeIcon icon="search" class="search-icon" />
                    <input
                      v-model="searchQueries['manHinh']"
                      @input="onSearchInput('manHinh', manHinhs, 'kichThuoc', 'maManHinh')"
                      type="text"
                      placeholder="Tìm theo mã, kích thước..."
                      class="search-input"
                      @click.stop
                    />
                  </div>
                  <div class="select-options">
                    <div
                      v-for="mh in filteredOptions['manHinh'] !== undefined
                        ? filteredOptions['manHinh']
                        : manHinhs"
                      :key="mh.id"
                      @click="selectOption('manHinh', mh.id, 'idManHinh')"
                      class="select-option"
                      :class="{ selected: form.idManHinh === mh.id }"
                    >
                      <span class="option-code" v-if="mh.maManHinh">{{ mh.maManHinh }} - </span>
                      <span class="option-name"
                        >{{ mh.kichThuoc }}{{ mh.doPhanGiai ? ' - ' + mh.doPhanGiai : '' }}</span
                      >
                    </div>
                    <div
                      v-if="
                        filteredOptions['manHinh'] !== undefined &&
                        filteredOptions['manHinh'].length === 0 &&
                        searchQueries['manHinh']
                      "
                      class="no-results"
                    >
                      Không tìm thấy kết quả
          ...
          <div
            class="form-group"
            :style="{ zIndex: activeDropdownKey === 'cameraSau' ? 10003 : 1 }"
          >
            <label>Camera sau</label>
            <div class="select-with-add">
              <div
                class="searchable-select"
                :style="{ zIndex: activeDropdownKey === 'cameraSau' ? 10001 : 10 }"
              >
                <div
                  class="searchable-select-input"
                  @click="toggleSearchableDropdown('cameraSau', cameraSaus, 'thongSo', 'maCamera')"
                >
                  <span class="select-text">{{
                    getDisplayText('cameraSau', cameraSaus, form.idCameraSau, 'thongSo') ||
                    'Chọn camera sau'
                  }}</span>
                  <span class="select-arrow">▼</span>
                </div>
                <div
                  v-if="showSearchableDropdowns['cameraSau']"
                  class="searchable-select-dropdown"
                  :style="{ zIndex: activeDropdownKey === 'cameraSau' ? 10002 : 10000 }"
                >
                  <div class="search-input-container">
                    <FontAwesomeIcon icon="search" class="search-icon" />
                    <input
                      v-model="searchQueries['cameraSau']"
                      @input="onSearchInput('cameraSau', cameraSaus, 'thongSo', 'maCamera')"
                      type="text"
                      placeholder="Tìm theo mã, thông số..."
                      class="search-input"
                      @click.stop
                  ...
                  <div class="select-options">
                    <div
                      v-for="cs in filteredOptions['cameraSau'] !== undefined
                        ? filteredOptions['cameraSau']
                        : cameraSaus"
                      :key="cs.id"
                      @click="selectOption('cameraSau', cs.id, 'idCameraSau')"
                      class="select-option"
                      :class="{ selected: form.idCameraSau === cs.id }"
                    >
                      <span class="option-code" v-if="cs.maCamera">{{ cs.maCamera }} - </span>
                      <span class="option-name">{{ cs.thongSo }}</span>
                    </div>
                    <div
                      v-if="
                        filteredOptions['cameraSau'] !== undefined &&
                        filteredOptions['cameraSau'].length === 0 &&
                        searchQueries['cameraSau']
                      "
                      class="no-results"
                    >
                      Không tìm thấy kết quả
          ...
          <div class="form-group" :style="{ zIndex: activeDropdownKey === 'chip' ? 10003 : 1 }">
            <label>Chip</label>
            <div class="select-with-add">
              <div
                class="searchable-select"
                :style="{ zIndex: activeDropdownKey === 'chip' ? 10001 : 10 }"
              >
                <div
                  class="searchable-select-input"
                  @click="toggleSearchableDropdown('chip', chips, 'tenChip', 'maChip')"
                >
                  <span class="select-text">{{
                    getDisplayText('chip', chips, form.idChip, 'tenChip') || 'Chọn chip'
                  }}</span>
                  <span class="select-arrow">▼</span>
                </div>
                <div
                  v-if="showSearchableDropdowns['chip']"
                  class="searchable-select-dropdown"
                  :style="{ zIndex: activeDropdownKey === 'chip' ? 10002 : 10000 }"
                >
                  <div class="search-input-container">
                    <FontAwesomeIcon icon="search" class="search-icon" />
                    <input
                      v-model="searchQueries['chip']"
                      @input="onSearchInput('chip', chips, 'tenChip', 'maChip')"
                      type="text"
                      placeholder="Tìm theo mã, tên..."
                      class="search-input"
                      @click.stop
                  ...
                  <div class="select-options">
                    <div
                      v-for="c in filteredOptions['chip'] !== undefined
                        ? filteredOptions['chip']
                        : chips"
                      :key="c.id"
                      @click="selectOption('chip', c.id, 'idChip')"
                      class="select-option"
                      :class="{ selected: form.idChip === c.id }"
                    >
                      <span class="option-code" v-if="c.maChip">{{ c.maChip }} - </span>
                      <span class="option-name">{{ c.tenChip }}</span>
                    </div>
                    <div
                      v-if="
                        filteredOptions['chip'] !== undefined &&
                        filteredOptions['chip'].length === 0 &&
                        searchQueries['chip']
                      "
                      class="no-results"
                    >
        ...
        <div class="form-column">
          <div class="form-group" :style="{ zIndex: activeDropdownKey === 'hang' ? 10003 : 1 }">
            <label>Hãng</label>
            <div class="select-with-add">
              <div
                class="searchable-select"
                :style="{ zIndex: activeDropdownKey === 'hang' ? 10001 : 10 }"
              >
                <div
                  class="searchable-select-input"
                  @click="toggleSearchableDropdown('hang', hangs, 'ten')"
                >
                  <span class="select-text">{{
                    getDisplayText('hang', hangs, form.idHang, 'ten') || 'Chọn hãng'
                  }}</span>
                  <span class="select-arrow">▼</span>
                </div>
                <div
                  v-if="showSearchableDropdowns['hang']"
                  class="searchable-select-dropdown"
                  :style="{ zIndex: activeDropdownKey === 'hang' ? 10002 : 10000 }"
                >
                  <div class="search-input-container">
                    <FontAwesomeIcon icon="search" class="search-icon" />
                    <input
                      v-model="searchQueries['hang']"
                      @input="onSearchInput('hang', hangs, 'ten')"
                      type="text"
                  ...
                  <div class="select-options">
                    <div
                      v-for="h in filteredOptions['hang'] !== undefined
                        ? filteredOptions['hang']
                        : hangs"
                      :key="h.id"
                      @click="selectOption('hang', h.id, 'idHang')"
                      class="select-option"
                      :class="{ selected: form.idHang === h.id }"
                    >
                      <span class="option-name">{{ h.ten }}</span>
                    </div>
                    <div
                      v-if="
                        filteredOptions['hang'] !== undefined &&
                        filteredOptions['hang'].length === 0 &&
                        searchQueries['hang']
                      "
                      class="no-results"
                    >
          ...
          <div class="form-group" :style="{ zIndex: activeDropdownKey === 'sim' ? 10003 : 1 }">
            <label>SIM</label>
            <div class="select-with-add">
              <div
                class="searchable-select"
                :style="{ zIndex: activeDropdownKey === 'sim' ? 10001 : 10 }"
              >
                <div
                  class="searchable-select-input"
                  @click="toggleSearchableDropdown('sim', sims, 'loaiSim')"
                >
                  <span class="select-text">{{
                    getDisplayText('sim', sims, form.idSim, 'loaiSim') || 'Chọn loại SIM'
                  }}</span>
                  <span class="select-arrow">▼</span>
                </div>
                <div
                  v-if="showSearchableDropdowns['sim']"
                  class="searchable-select-dropdown"
                  :style="{ zIndex: activeDropdownKey === 'sim' ? 10002 : 10000 }"
                >
                  <div class="search-input-container">
                    <FontAwesomeIcon icon="search" class="search-icon" />
                    <input
                      v-model="searchQueries['sim']"
                      @input="onSearchInput('sim', sims, 'loaiSim')"
                      type="text"
                      placeholder="Tìm theo loại SIM..."
                      class="search-input"
                      @click.stop
                    />
                  </div>
                  <div class="select-options">
                    <div
                      v-for="sim in filteredOptions['sim'] !== undefined
                        ? filteredOptions['sim']
                        : sims"
                      :key="sim.id"
                      @click="selectOption('sim', sim.id, 'idSim')"
                      class="select-option"
                      :class="{ selected: form.idSim === sim.id }"
                    >
                      <span class="option-name">{{ sim.loaiSim }}</span>
                    </div>
                    <div
                      v-if="
                        filteredOptions['sim'] !== undefined &&
                        filteredOptions['sim'].length === 0 &&
                        searchQueries['sim']
                      "
                      class="no-results"
                    >
          ...
          <div class="form-group" :style="{ zIndex: activeDropdownKey === 'cpu' ? 10003 : 1 }">
            <label>CPU</label>
            <div class="select-with-add">
              <div
                class="searchable-select"
                :style="{ zIndex: activeDropdownKey === 'cpu' ? 10001 : 10 }"
              >
                <div
                  class="searchable-select-input"
                  @click="toggleSearchableDropdown('cpu', cpus, 'tenCpu', 'maCpu')"
                >
                  <span class="select-text">{{
                    getDisplayText('cpu', cpus, form.idCpu, 'tenCpu') || 'Chọn CPU'
                  }}</span>
                  <span class="select-arrow">▼</span>
                </div>
                <div
                  v-if="showSearchableDropdowns['cpu']"
                  class="searchable-select-dropdown"
                  :style="{ zIndex: activeDropdownKey === 'cpu' ? 10002 : 10000 }"
                >
                  <div class="search-input-container">
                    <FontAwesomeIcon icon="search" class="search-icon" />
                    <input
                      v-model="searchQueries['cpu']"
                      @input="onSearchInput('cpu', cpus, 'tenCpu', 'maCpu')"
                      type="text"
                      placeholder="Tìm theo mã, tên..."
                      class="search-input"
                      @click.stop
                  ...
                  <div class="select-options">
                    <div
                      v-for="cpu in filteredOptions['cpu'] !== undefined
                        ? filteredOptions['cpu']
                        : cpus"
                      :key="cpu.id"
                      @click="selectOption('cpu', cpu.id, 'idCpu')"
                      class="select-option"
                      :class="{ selected: form.idCpu === cpu.id }"
                    >
                      <span class="option-code" v-if="cpu.maCpu">{{ cpu.maCpu }} - </span>
                      <span class="option-name">{{ cpu.tenCpu }}</span>
                    </div>
                    <div
                      v-if="
                        filteredOptions['cpu'] !== undefined &&
                        filteredOptions['cpu'].length === 0 &&
                        searchQueries['cpu']
                      "
                      class="no-results"
                    >
          ...
          <div class="form-group" :style="{ zIndex: activeDropdownKey === 'gpu' ? 10003 : 1 }">
            <label>GPU</label>
            <div class="select-with-add">
              <div
                class="searchable-select"
                :style="{ zIndex: activeDropdownKey === 'gpu' ? 10001 : 10 }"
              >
                <div
                  class="searchable-select-input"
                  @click="toggleSearchableDropdown('gpu', gpus, 'tenGpu', 'maGpu')"
                >
                  <span class="select-text">{{
                    getDisplayText('gpu', gpus, form.idGpu, 'tenGpu') || 'Chọn GPU'
                  }}</span>
                  <span class="select-arrow">▼</span>
                </div>
                <div
                  v-if="showSearchableDropdowns['gpu']"
                  class="searchable-select-dropdown"
                  :style="{ zIndex: activeDropdownKey === 'gpu' ? 10002 : 10000 }"
                >
                  <div class="search-input-container">
                    <FontAwesomeIcon icon="search" class="search-icon" />
                    <input
                      v-model="searchQueries['gpu']"
                      @input="onSearchInput('gpu', gpus, 'tenGpu', 'maGpu')"
                      type="text"
                      placeholder="Tìm theo mã, tên..."
                      class="search-input"
                      @click.stop
                  ...
                  <div class="select-options">
                    <div
                      v-for="gpu in filteredOptions['gpu'] !== undefined
                        ? filteredOptions['gpu']
                        : gpus"
                      :key="gpu.id"
                      @click="selectOption('gpu', gpu.id, 'idGpu')"
                      class="select-option"
                      :class="{ selected: form.idGpu === gpu.id }"
                    >
                      <span class="option-code" v-if="gpu.maGpu">{{ gpu.maGpu }} - </span>
                      <span class="option-name">{{ gpu.tenGpu }}</span>
                    </div>
                    <div
                      v-if="
                        filteredOptions['gpu'] !== undefined &&
                        filteredOptions['gpu'].length === 0 &&
                        searchQueries['gpu']
                      "
                      class="no-results"
                    >
    ...
    <div class="form-card" style="z-index: 0">
      <div class="form-group">
    ...
    <div>
      <h2 class="section-title">{{ isCreate ? 'THÊM' : 'QUẢN LÝ' }} PHIÊN BẢN</h2>
      <div class="form-card">
        <div class="form-row">
          <div class="form-group">
            <label>RAM</label>
              ...
              <div v-if="showRamDropdown" class="multi-select-dropdown">
                <div class="multi-select-options">
                  <div
                    ...
                    @click="toggleRam(r.id)"
                  >
                    <input
                      type="checkbox"
                      :checked="isRamSelected(r.id)"
                      @change="toggleRam(r.id)"
                    />
                    <span>{{ r.tenRam }}</span>
                  </div>
                </div>
              ...
              <div v-if="showRomDropdown" class="multi-select-dropdown">
                <div class="multi-select-options">
                  <div
                    ...
                    :key="r.id"
                    class="multi-select-option"
                    @click="toggleRom(r.id)"
                  >
                    <input
                      type="checkbox"
                      :checked="isRomSelected(r.id)"
                      @change="toggleRom(r.id)"
                    />
              ...
              <div class="multi-select" @click="openMauSacDropdown()">
                <div class="multi-select-content">
                  <div v-if="variantForm.selectedMauSacs.length > 0" class="selected-colors">
                    <div
                      v-for="mauSacId in variantForm.selectedMauSacs"
                      :key="mauSacId"
                      class="selected-color-item"
                    >
                      <div
                        class="color-indicator"
                        :class="`color-${mauSacId}`"
                        :style="{ backgroundColor: getColorCode(mauSacId) }"
                      ></div>
              ...
              <div v-if="showMauSacDropdown" class="multi-select-dropdown">
                <div class="multi-select-options">
                  <div
                    ...
                    :key="m.id"
                    class="multi-select-option"
                    @click="toggleMauSac(m.id)"
                  >
                    <input
                      type="checkbox"
                      :checked="isMauSacSelected(m.id)"
                      @change="toggleMauSac(m.id)"
                    />
                    <div class="color-option">
                      <div
                        class="color-indicator"
                        :class="`color-${m.id}`"
                        :style="{ backgroundColor: getColorCode(m.id) }"
                      ></div>
                      <span>{{ m.tenMau }}</span>
                    </div>
                  </div>
        ...
        <div class="form-row variant-actions-row">
          <div class="form-group variant-actions-group">
            <button class="btn-secondary" @click="addVariant">
              <FontAwesomeIcon icon="plus" class="btn-icon" style="margin-right: 8px" />
              Thêm phiên bản
            </button>
            <button class="btn-clear" @click="clearVariantSelections">
              <FontAwesomeIcon icon="trash" class="btn-icon" style="margin-right: 8px" />
              Xóa lựa chọn
            </button>
          </div>
        </div>

        <div v-if="variants.length > 0" class="variants-display">
            ...
            <div class="variant-group-header">
              ...
              <div class="price-input-section">
                <input
                  ...
                  type="number"
                />
                <button class="btn-apply-price" @click="applyPriceToAll(group.key)">
                  <FontAwesomeIcon icon="check" class="btn-icon btn-icon--md" /> Áp dụng
                </button>
                  ...
                  <tr v-for="(v, i) in group.variants" :key="i" class="variant-row">
                    ...
                    <td>
                      <div class="variant-color">
                        <div
                          class="color-indicator"
                          :class="`color-${v.idMauSac}`"
                          :style="{ backgroundColor: getColorCode(v.idMauSac) }"
                        ></div>
                    ...
                    <td>
                      <div class="quantity-display">
                        <span class="imei-count">
                          {{
                            variants[v.originalIndex].imeis &&
                            variants[v.originalIndex].imeis.length > 0
                              ? variants[v.originalIndex].imeis.length
                              : 0
                          }}
                          IMEI
                        </span>
                    ...
                    <td>
                      <div class="variant-actions">
                        ...
                        <input
                          ...
                          style="display: none"
                        />
                        <button class="btn-import-excel" @click="importFromExcel(v.originalIndex)">
                          <FontAwesomeIcon icon="file-import" class="btn-icon btn-icon--md" /> Nhập
                        </button>
                      </div>
                    </td>
                  </tr>
    ...
    <div v-if="variants.length" class="image-upload-section">
      <h2 class="section-title">📷 {{ isCreate ? 'THÊM' : 'QUẢN LÝ' }} HÌNH ẢNH THEO MÀU SẮC</h2>
      <div class="color-image-grid">
        <div
          v-for="colorGroup in colorImageGroups"
        ...
        >
          <div class="card-header">
            <h3 class="color-title">
              <div class="color-title-content">
                <span
                  class="color-indicator"
                  :class="`color-${colorGroup.colorId}`"
                  :style="{ backgroundColor: getColorCode(colorGroup.colorId) }"
                ></span>
          ...
          <div class="card-content">
            ...
            <div
              v-if="colorGroup.images && colorGroup.images.length > 0"
              class="image-preview-grid"
            >
              <div
                v-for="(image, imageIndex) in colorGroup.images"
                :key="imageIndex"
              ...
              >
                ...
                <button
                  ...
                  @click="removeColorImage(colorGroup.colorId, imageIndex)"
                >
                  ×
                </button>
              </div>
            ...
            <div class="upload-section">
              <input
                ...
                style="display: none"
              />
              <button class="upload-btn" @click="triggerColorFileInput(colorGroup.colorId)">
    ...
    <div class="form-actions" style="margin-top: 12px">
      ...
      <button
        class="btn-primary"
        :disabled="isCreate ? !canSaveCreate : !canSaveUpdate"
        @click="save"
      >
  ...
  <div v-if="showModal" class="modal">
    <div class="modal-content">
      ...
      <div class="modal-body">
        <div v-for="(value, field) in modalInputs" :key="field" class="form-group">
          ...
          <input
            ...
            :ref="
              (field as string) === 'ten' ||
              (field as string) === 'tenDanhMuc' ||
              (field as string) === 'tenHeDieuHanh' ||
              (field as string) === 'tenChip' ||
              (field as string) === 'tenCpu' ||
              (field as string) === 'tenGpu' ||
              (field as string) === 'tenRam' ||
              (field as string) === 'tenMau'
                ? 'modalInputRef'
                : null
            "
          />
      ...
      <div class="modal-footer">
        ...
        <button class="btn-primary" @click="saveModal" :disabled="!isModalValid() || modalLoading">
    ...
    <div class="modal-content imei-modal">
      ...
      <div class="modal-body">
        ...
        <div class="imei-input-section">
          <div class="imei-input-header">
            <h4>Nhập IMEI</h4>
            <button class="btn-scan-qr" @click="startQRScanner" type="button">
              <FontAwesomeIcon icon="qrcode" class="btn-icon" />
              Quét QR
            </button>
          </div>
          <textarea
        ...
        <div class="imei-list-section">
          ...
          <div v-else class="imei-list">
            ...
            <div
              v-for="(imei, index) in getInputImeis()"
              :key="`input-${index}`"
              class="imei-item"
              :class="getImeiValidationClass(imei)"
            >
              ...
              <div class="imei-actions">
                ...
                <button
                  class="btn-remove-imei-input"
                  @click="removeImeiFromInput(index)"
                  title="Xóa IMEI"
                >
                  ×
                </button>
            ...
            <div
              v-for="(imei, index) in getCurrentImeis()"
              :key="`saved-${index}`"
              class="imei-item imei-item-saved"
            >
              <span class="imei-text">{{ imei }}</span>
        ...
        <div class="excel-import-section">
          ...
          <div class="file-input-container">
            <input
              ...
              class="excel-file-input"
            />
            <button class="btn-choose-file" @click="triggerExcelFileInput">Choose File</button>
      ...
      <div class="modal-footer imei-modal-footer">
        <button class="btn-secondary imei-modal-btn" @click="clearImeiInput">Xóa</button>
        <button class="btn-secondary imei-modal-btn" @click="closeImeiModal">Đóng</button>
        <button
          class="btn-primary imei-modal-btn"
          @click="saveImei"
  ...
  <!-- QR Scanner Modal -->
  <div v-if="showQRScannerModal" class="modal">
    <div class="modal-content qr-scanner-modal">
      ...
      <div class="modal-body">
        <div class="qr-scanner-container">
          <video id="imei-qr-scanner-video" class="qr-scanner-video"></video>
          <div v-if="!qrScanner" class="qr-scanner-loading">
            <p>Đang khởi động camera...</p>
          </div>
        </div>
        <p class="qr-scanner-hint">Đưa camera vào mã QR IMEI để quét. Có thể quét nhiều lần.</p>
        <p class="qr-scanner-warning" v-if="!isSecureContext">
          ⚠️ Camera chỉ hoạt động trên HTTPS hoặc localhost. Vui lòng sử dụng HTTPS hoặc nhập IMEI
          thủ công.
        </p>
      </div>
    </div>
...
.btn-clear:hover {
  ...
  transform: translateY(-2px);
  box-shadow: 0 8px 15px -3px rgba(71, 85, 105, 0.4);
}
.btn-clear:active {
  transform: translateY(0);
...
.variants-table th:nth-child(8),
.variants-table td:nth-child(8) {
  width: 80px;
  min-width: 70px;
  text-align: center;
} /* THAO TÁC */
...
.variant-actions-group .btn-secondary,
.variant-actions-group .btn-clear {
  flex: 0 1 auto;
  width: auto;
  min-width: 200px;
  max-width: 250px;
