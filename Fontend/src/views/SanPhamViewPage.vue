      ...
      <div class="main-content">
        ...
        <div class="table-header">
          <div class="table-title">
            <button @click="goBack" class="btn-back" title="Quay lại trang sản phẩm">
              <font-awesome-icon icon="arrow-left" />
            </button>
            <h2>Chi tiết Sản phẩm</h2>
            <button @click="goToEdit" class="btn-edit" title="Cập nhật sản phẩm">
        ...
        <div class="table-container">
          <table>
            ...
            <tbody>
              <tr
                v-for="(ct, index) in paginatedChiTiets"
                :key="ct.id"
                class="variant-row"
                :class="{ selected: selectedVariant?.id === ct.id }"
                @click="selectedVariant = ct"
              >
                ...
                <td>{{ ct.maCtsp || ct.maChiTietSanPham || `CTSP${ct.id}` }}</td>
                <td>{{ product?.tenSanPham || 'Chưa có tên' }}</td>
                <td>
                  <div class="product-image-cell">
                    <img
                      v-if="getProductImage(ct)"
                      :src="getProductImage(ct)"
                      :alt="`Hình ảnh ${getRamName(ct)} ${getRomName(ct)} ${getMauName(ct)}`"
                      class="product-thumbnail"
                      @error="handleImageError"
                      @click="openImageModal(getProductImage(ct)!)"
                    />
                    <span v-else class="no-image">Chưa có ảnh</span>
                  </div>
                </td>
                <td>{{ getRamName(ct) }}</td>
                <td>{{ getRomName(ct) }}</td>
                ...
                <td>
                  <div class="text-right">
                    <div
                      v-if="getGiaGoc(ct) > getGiaBan(ct)"
                      class="text-xs line-through text-slate-400"
                    >
                      {{ formatPrice(getGiaGoc(ct)) }}
                    </div>
                    <div class="font-semibold">{{ formatPrice(getGiaBan(ct)) }}</div>
                    <div v-if="getGiamPhanTram(ct)" class="text-xs text-green-600">
                      -{{ Math.round(getGiamPhanTram(ct) as number) }}%
                    </div>
                  </div>
                </td>
                <td>
                  <div
                    class="text-xs"
                    :class="getGiamPhanTram(ct) ? 'text-green-600' : 'text-slate-400'"
                  >
                    <span v-if="getGiamPhanTram(ct)"
                      >{{ Math.round(getGiamPhanTram(ct) as number) }}%</span
                    >
                    <span v-else>—</span>
                  </div>
                </td>
                ...
                <td>
                  <span
                    class="status-badge"
                    :class="(ct.trangThai ?? 0) === 1 ? 'badge-active' : 'badge-inactive'"
                  >
                  ...
                  <div class="action-buttons">
                    ...
                    <div class="toggle-container">
                      <label
                        class="toggle-switch"
                        :title="
                          getSoLuong(ct) === 0 && (ct.trangThai ?? 0) === 0
                            ? 'Biến thể có số lượng = 0. Vui lòng thêm IMEI sau khi kích hoạt.'
                            : (ct.trangThai ?? 0) === 1
                              ? 'Gạt để vô hiệu hóa'
                              : 'Gạt để kích hoạt'
                        "
                      >
                        <input
                          type="checkbox"
                          :checked="(ct.trangThai ?? 0) === 1"
                          @change="toggleChiTietStatus(ct)"
                        />
              ...
              <div class="info-grid">
                ...
                <div class="info-item">
                  ...
                  <span
                    class="status-badge"
                    :class="
                      (selectedVariant.trangThai ?? 0) === 1 ? 'badge-active' : 'badge-inactive'
                    "
                  >
              ...
              <div class="info-grid">
                ...
                <div class="info-item">
                  ...
                  <div class="color-info">
                    ...
                    <div
                      v-if="selectedVariant?.mauSac?.maHex"
                      class="color-preview"
                    ...
                    <div v-else class="imei-list">
                      <div class="imei-item" v-for="(imei, index) in variantImeis" :key="imei.id">
                        ...
                        <div class="imei-status">
                          <span
                            class="status-badge"
                            :class="imei.trangThai === 1 ? 'badge-active' : 'badge-inactive'"
                          >
                            {{ imei.trangThai === 1 ? 'Hoạt động' : 'Ngừng hoạt động' }}
                          </span>
                          <!-- Toggle switch chỉ hiển thị cho IMEI đang hoạt động -->
                          <label
                            v-if="imei.trangThai === 1"
                            class="toggle-switch"
                            :title="'Gạt để ngừng hoạt động IMEI này'"
                          >
                            <input
                              type="checkbox"
                              :checked="true"
                              @change="toggleImeiStatus(imei)"
                              class="toggle-input"
                            />
                            <span class="toggle-slider"></span>
                          </label>
              ...
              <div class="info-grid">
                <div class="info-item">
                  ...
                  <span class="info-value price-value">
                    {{
                      selectedVariant.giaNhap
                        ? formatPrice(selectedVariant.giaNhap)
                        : 'Chưa cập nhật'
                    }}
                  </span>
                </div>
                <div class="info-item">
                  ...
                  <span
                    ...
                    :class="
                      getGiaGoc(selectedVariant) > getGiaBan(selectedVariant)
                ...
                <div class="info-item" v-if="getGiamPhanTram(selectedVariant)">
                  ...
                  <span class="info-value">
                    -{{ Math.round(getGiamPhanTram(selectedVariant) as number) }}%
                    {{ selectedVariant.tenDotGiam ? `(${selectedVariant.tenDotGiam})` : '' }}
                  </span>
                </div>
                <div class="info-item">
...
const allFilteredChiTiets = computed(() => {
  ...
  if (attributeFilter.value) {
    ...
    filtered = filtered.filter((ct) => {
      if (attr === 'ram')
        return (
          (getRamName(ct) || '').toLowerCase().includes((searchText.value || '').toLowerCase()) ||
          searchText.value === ''
        )
      if (attr === 'rom')
        return (
          (getRomName(ct) || '').toLowerCase().includes((searchText.value || '').toLowerCase()) ||
          searchText.value === ''
        )
      if (attr === 'mau')
        return (
          (getMauName(ct) || '').toLowerCase().includes((searchText.value || '').toLowerCase()) ||
          searchText.value === ''
        )
      return true
    })
  ...
  if (selectedRamId.value) {
    filtered = filtered.filter(
      (ct) =>
        ct.ramId === selectedRamId.value ||
        getRamName(ct) === rams.value.find((r) => r.id === selectedRamId.value)?.tenRam,
    )
  }
  if (selectedRomId.value) {
    filtered = filtered.filter(
      (ct) =>
        ct.romId === selectedRomId.value ||
        getRomName(ct) === roms.value.find((r) => r.id === selectedRomId.value)?.dungLuong,
    )
  }
  if (selectedMauId.value) {
    filtered = filtered.filter(
      (ct) =>
        ct.mauSacId === selectedMauId.value ||
        getMauName(ct) === mauSacs.value.find((m) => m.id === selectedMauId.value)?.tenMau,
    )
  }
...
const isAllSelected = computed(() => {
  return (
    paginatedChiTiets.value.length > 0 &&
...
async function loadChiTiets() {
  try {
    ...
    // Use admin endpoint to get ALL variants (including inactive ones)
    const response = await api.get(`/api/san-pham/${productId.value}/chi-tiet/all`)
    ...
    // Kiểm tra và xử lý hình ảnh từ dữ liệu ban đầu nếu có
    allChiTiets.forEach((ct: any) => {
      // Nếu có hinhAnhs trong dữ liệu
      if (ct.hinhAnhs && Array.isArray(ct.hinhAnhs) && ct.hinhAnhs.length > 0) {
        const firstImage = ct.hinhAnhs[0]
        if (firstImage && firstImage.urlAnh) {
          ct.hinhAnh = firstImage.urlAnh
          ct.imageUrls = ct.hinhAnhs.map((img: any) => img.urlAnh || img)
        }
      }
      // Nếu có imageUrls trong dữ liệu
      else if (ct.imageUrls && Array.isArray(ct.imageUrls) && ct.imageUrls.length > 0) {
        ct.hinhAnh =
          typeof ct.imageUrls[0] === 'string' ? ct.imageUrls[0] : ct.imageUrls[0]?.urlAnh || null
      }
    })
    ...
    chiTiets.value.forEach((ct, index) => {
      console.log(`CTSP ${index}:`, {
        ...
        // Debug hình ảnh
        hinhAnh: (ct as any).hinhAnh,
        imageUrls: (ct as any).imageUrls,
        images: (ct as any).images,
        fullData: ct,
      })
    })
    ...
    // KHÔNG tự động vô hiệu hóa - để admin chủ động quản lý
    // await autoDisableZeroQuantityProducts()
...
async function loadAllVariantImages() {
  // Load images for all variants in parallel - sử dụng logic giống như trong modal
  const imagePromises = chiTiets.value.map(async (ct) => {
    try {
      const response = await api.get(`/api/san-pham/chi-tiet/${ct.id}/images`)
      const images = Array.isArray(response.data) ? response.data : []

      console.log(`📸 Loading images for variant ${ct.id}:`, {
        responseData: response.data,
        imagesArray: images,
        firstImage: images[0],
      })

      // Lưu hình ảnh đầu tiên vào chi tiết sản phẩm - logic giống modal
      if (images.length > 0) {
        // Lưu toàn bộ mảng images vào variantImages (giống như trong modal)
        ;(ct as any).variantImages = images
        // Lấy hình ảnh đầu tiên - sử dụng logic giống modal: image.urlAnh
        const firstImage = images[0]
        if (firstImage && firstImage.urlAnh) {
          ;(ct as any).hinhAnh = firstImage.urlAnh(ct as any).imageUrls = images.map(
            (img: any) => img.urlAnh,
          )
          console.log(`✅ Set image for variant ${ct.id}:`, (ct as any).hinhAnh)
        } else {
          console.warn(`⚠️ First image does not have urlAnh for variant ${ct.id}:`, firstImage)
        }
      } else {
        console.log(`ℹ️ No images found for variant ${ct.id}`)
      }
    } catch (error) {
      console.warn(`Failed to load images for variant ${ct.id}:`, error)
      // Không throw error, chỉ log warning
    }
  })

  await Promise.all(imagePromises)
  console.log('📸 Finished loading all variant images')
}
...
async function toggleChiTietStatus(ct: any) {
  try {
    const soLuong = getSoLuong(ct)
    const newStatus = (ct.trangThai ?? 0) === 1 ? 0 : 1

    // Cảnh báo nếu bật variant có số lượng = 0 (nhưng vẫn cho phép)
    if (soLuong === 0 && newStatus === 1) {
      toastRef.value?.warning(
        'Cảnh báo',
        'Biến thể có số lượng = 0. Vui lòng thêm IMEI trong trang chỉnh sửa sản phẩm.',
      )
    }
    ...
    if (response.data) {
      ...
      toastRef.value?.success(
        'Thành công',
        newStatus === 1 ? 'Đã kích hoạt phiên bản!' : 'Đã vô hiệu hóa phiên bản!',
      )
    } else {
...
// Tự động vô hiệu hóa các sản phẩm có số lượng = 0
async function autoDisableZeroQuantityProducts() {
  const disablePromises = chiTiets.value
    .filter((ct) => {
      const soLuong = getSoLuong(ct)
      return soLuong === 0 && (ct.trangThai ?? 0) === 1
    })
    .map(async (ct) => {
      try {
        const cleanId = parseInt(String(ct.id).replace(/[^\d]/g, ''), 10)
        if (isNaN(cleanId)) {
          console.warn(`Invalid ID for auto-disable: ${ct.id}`)
          return
        }

        await api.put(`/api/san-pham/chi-tiet/${cleanId}/status`, { trangThai: 0 })
        ct.trangThai = 0
        console.log(`✅ Auto-disabled product ${ct.id} due to zero quantity`)
      } catch (error) {
        console.warn(`Failed to auto-disable product ${ct.id}:`, error)
      }
    })
  ...
  if (disablePromises.length > 0) {
    ...
    toastRef.value?.info(
      'Thông báo',
      `Đã tự động vô hiệu hóa ${disablePromises.length} sản phẩm có số lượng = 0`,
    )
  }
}
...
function exportToExcel() {
  try {
    ...
    const hangsToExport =
      selectedChiTietsComputed.value.length > 0 ? selectedChiTietsComputed.value : chiTiets.value
...
function getSoLuong(ct: any): number {
  const soLuong = (ct?.soLuong ?? 0) as number
  // Nếu số lượng < 0, trả về 0 (không cho phép số lượng âm)
  return soLuong < 0 ? 0 : soLuong
}
...
async function openVariantModal(ct: any) {
  ...
  await Promise.all([loadVariantImeis(ct.id), loadVariantImages(ct.id)])
}
...
async function loadVariantImeis(chiTietId: number) {
  try {
    ...
    // Hiển thị tất cả IMEI (cả hoạt động và ngừng hoạt động)
    variantImeis.value = allImeis
    console.log('📱 Loaded all IMEIs:', variantImeis.value)
  } catch (error) {
...
async function toggleImeiStatus(imei: any) {
  try {
    const newStatus = imei.trangThai === 1 ? 0 : 1

    // Gọi API để cập nhật trạng thái
    await api.put(`/api/imei/${imei.id}/status?value=${newStatus}`)

    // Cập nhật trạng thái trong local state
    imei.trangThai = newStatus

    // Nếu đang vô hiệu hóa IMEI, gửi event để thông báo cho POS page
    if (newStatus === 0) {
      // Lấy chiTietSanPhamId từ selectedVariant hoặc từ imei
      const chiTietSanPhamId = selectedVariant.value?.id || imei.idCtsp || imei.chiTietSanPham?.id

      // Gửi event qua localStorage để POS page có thể lắng nghe
      const event = new CustomEvent('imei-deactivated', {
        detail: {
          imeiId: imei.id,
          imei: imei.imei,
      ...
      // Cũng lưu vào localStorage để các tab khác có thể nhận
      ...
      // Trigger storage event để các tab khác nhận được
      window.dispatchEvent(
        new StorageEvent('storage', {
          key: 'imei-deactivated',
    ...
    toastRef.value?.success(
      'Thành công',
      `Đã ${newStatus === 0 ? 'ngừng hoạt động' : 'kích hoạt'} IMEI ${imei.imei}`,
    )
  } catch (error) {
    console.error('Error updating IMEI status:', error)
    toastRef.value?.error('Lỗi', 'Không thể cập nhật trạng thái IMEI')
  }
}
...
function goToEdit() {
  if (productId.value) {
    router.push(`/san-pham/form/${productId.value}`)
  } else {
    router.push('/san-pham/form/')
  }
}

function getProductImage(ct: ChiTietSanPham): string | null {
  // Sử dụng logic giống như trong modal: variantImages[0].urlAnh
  // Nếu có variantImages (đã load từ API)
  if (
    (ct as any).variantImages &&
    Array.isArray((ct as any).variantImages) &&
    (ct as any).variantImages.length > 0
  ) {
    const firstImage = (ct as any).variantImages[0]
    if (firstImage && firstImage.urlAnh) {
      return firstImage.urlAnh
    }
  }

  // Nếu có hinhAnh đã được set (từ loadAllVariantImages)
  if ((ct as any).hinhAnh) {
    return (ct as any).hinhAnh
  }

  // Nếu có imageUrls trong dữ liệu, lấy hình ảnh đầu tiên
  if (
    (ct as any).imageUrls &&
    Array.isArray((ct as any).imageUrls) &&
    (ct as any).imageUrls.length > 0
  ) {
    const firstImage = (ct as any).imageUrls[0]
    // Nếu là string (URL), trả về trực tiếp
    if (typeof firstImage === 'string') {
      return firstImage
    }
    // Nếu là object có urlAnh, lấy urlAnh
    if (typeof firstImage === 'object' && firstImage !== null && firstImage.urlAnh) {
      return firstImage.urlAnh
    }
  }

  // Nếu có hình ảnh từ product
...
.action-buttons .toggle-switch.disabled {
  opacity: 0.5;
  cursor: not-allowed;
}

.action-buttons .toggle-switch.disabled .toggle-slider {
  cursor: not-allowed;
  background-color: #cbd5e1;
}

.action-buttons .toggle-switch input:disabled + .toggle-slider {
  cursor: not-allowed;
  background-color: #cbd5e1;
  opacity: 0.6;
}

.action-buttons .toggle-switch input:disabled:checked + .toggle-slider {
