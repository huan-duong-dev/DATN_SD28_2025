                  ...
                  <div class="toggle-container">
                    <label class="toggle-switch" :class="{ 'disabled': isVoucherExpired(voucher) || voucher.soLuongDung <= 0 }">
                      <input
                        ...
                        @click.prevent="toggleStatus(voucher.id)"
                      />
...
import { onMounted, onActivated, watch, ref, computed } from 'vue'
...
async function loadVouchers() {
  loading.value = true
  try {
    console.log('Loading vouchers from /api/phieu-giam-gia?all=true...')
    // Sử dụng ?all=true để lấy TẤT CẢ voucher (cả public và private, cả active và inactive)
    const { data } = await api.get<PhieuGiamGia[]>('/api/phieu-giam-gia?all=true')
...
async function toggleStatus(id: number) {
  ...
  if (!voucher) {
    toastRef.value?.error('Lỗi', 'Không tìm thấy phiếu giảm giá')
    return
  }
  ...
  if (startDate > endDate) {
    toastRef.value?.error('Thời gian không hợp lệ', 'Ngày bắt đầu không thể lớn hơn ngày kết thúc. Vui lòng kiểm tra lại thông tin phiếu giảm giá.')
    return
  }

  // Nếu đang cố gắng kích hoạt (trangThai = 0 -> 1)
  if (voucher.trangThai === 0) {
    ...
    if (isVoucherNotStarted(voucher)) {
      toastRef.value?.warning('Chưa thể kích hoạt', `Voucher chưa đến thời gian hiệu lực. Ngày bắt đầu: ${formatDate(voucher.ngayBatDau)}`)
      return
    }

    // Kiểm tra nếu thời gian hiện tại không nằm trong khoảng thời gian hiệu lực
    if (now < startDate || now > endDate) {
      toastRef.value?.error('Thời gian không hợp lệ', `Không thể kích hoạt voucher. Thời gian hiệu lực: ${formatDate(voucher.ngayBatDau)} - ${formatDate(voucher.ngayKetThuc)}`)
      return
    }
  }
...
// Reload vouchers when component is mounted or activated
onMounted(loadVouchers)
onActivated(loadVouchers)

// Watch for route changes to reload when navigating back from form
watch(() => route.path, (newPath, oldPath) => {
  // Reload if navigating back to voucher list from form or detail page
  if (newPath === '/voucher' && (oldPath?.startsWith('/voucher/form') || oldPath?.startsWith('/voucher/detail'))) {
    console.log('Reloading vouchers after navigation from form/detail')
    loadVouchers()
  }
})
</script>
