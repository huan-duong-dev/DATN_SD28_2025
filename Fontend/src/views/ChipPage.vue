              ...
              <tr>
                <th class="checkbox-column">
                  <input
                    ...
                    :checked="
                      selectedItems.length === paginatedChips.length && paginatedChips.length > 0
                    "
                  />
                </th>
                <th>STT</th>
        ...
        <div class="pagination">
          <div class="pagination-info">
            <span
              ...
              {{ Math.min(currentPage * itemsPerPage, totalItems) }} trong {{ totalItems }} kết
              quả</span
            >
          </div>
              ...
              <div class="form-fields">
                <div class="form-group">
                  ...
                  <div class="input-group">
                    <input
                      ...
                      class="form-input"
                      @blur="formData.maChip = normalizeCode(formData.maChip)"
                      required
                    />
                    <button
                      @click="generateMaChipAuto"
                  ...
                  <div class="field-hint">
                    ⚠️ Mã chỉ chứa chữ viết hoa, số, gạch ngang và gạch dưới
                  </div>
                </div>
                ...
                <div class="form-group">
                  ...
                  <input
                    ...
                    class="form-input"
                    @blur="formData.tenChip = normalizeName(formData.tenChip)"
                    required
                  />
...
const viewingChip = ref<Chip | null>(null)
...
const filteredChips = computed(() => {
  ...
  if (searchText.value) {
    ...
    filtered = filtered.filter(
      (chip) =>
        chip.tenChip?.toLowerCase().includes(search) || chip.maChip?.toLowerCase().includes(search),
    )
  }
...
async function loadChips() {
  try {
    ...
    // Sort chips by creation date (newest first)
    chips.value = allChips.sort((a: any, b: any) => {
...
function viewChip(chip: Chip) {
  viewingChip.value = chip
  showViewModal.value = true
}

const viewFields = [
  { key: 'maChip', label: 'Mã Chip', type: 'text' },
  { key: 'tenChip', label: 'Tên Chip', type: 'text' },
...
async function handleFormSubmit() {
  ...
  if (formData.value.maChip) {
    formData.value.maChip = normalizeCode(formData.value.maChip)
  }
  if (formData.value.tenChip) {
    formData.value.tenChip = normalizeName(formData.value.tenChip)
  }
  ...
  const validation = await validateAttributeForm(
    formData.value,
    { code: 'maChip', name: 'tenChip' },
    { endpoint: '/api/chip', nameLabel: 'chip', editingId: editingChip.value?.id },
    toastRef.value,
  )
...
async function toggleChipStatus(chip: Chip) {
  try {
    ...
    toastRef.value?.success(
      'Thành công',
      `Đã ${newStatus === 1 ? 'kích hoạt' : 'ngừng hoạt động'} chip`,
    )
  } catch (error) {
