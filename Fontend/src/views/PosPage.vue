<template>
  <div class="h-screen flex flex-col bg-slate-100 overflow-hidden">
    <!-- PosHeader Component (giống các trang khác) -->
    <PosHeader 
      page-title="Bán hàng"
      page-subtitle="Hệ thống bán hàng POS"
    />

    <!-- Invoice Tabs Section -->
    <div class="bg-white shadow-sm border-b border-slate-200" style="margin-top: 80px;">
          ...
          <div class="flex items-center space-x-4">
            ...
            <div class="relative flex items-center space-x-2">
              ...
              <button
                ...
                class="flex items-center justify-center w-10 h-10 bg-blue-500 hover:bg-blue-600 text-white rounded-lg transition-all shadow-sm hover:shadow-md"
                title="Quét QR IMEI sản phẩm"
              >
                ...
                >
                  ...
                  <div class="flex-1 ml-3">
                    ...
                    <p class="text-xs text-slate-500">
                      ...
                      <span v-if="product.tenRam && product.tenRom"> • </span>
                      <span v-if="product.tenRom">{{ product.tenRom }}</span>
                      <span v-if="(product.tenRam || product.tenRom) && product.tenMauSac"> • </span>
            ...
            <button
              @click="addNewInvoice"
              class="flex items-center justify-center w-10 h-10 bg-gradient-to-r from-orange-500 to-orange-600 hover:from-orange-600 hover:to-orange-700 text-white rounded-lg transition-all shadow-sm hover:shadow-md"
    ...
    <div class="flex-1 flex overflow-hidden">
          ...
          <div class="space-y-3">
            ...
            >
              ...
              <div class="flex-1 ml-3">
                ...
                <p class="text-xs text-slate-500">
                  <span v-if="item.tenHang">{{ item.tenHang }}</span>
                  <span v-if="item.tenHang && (item.tenRam || item.tenRom || item.tenMauSac)"> • </span>
                  <span v-if="item.tenRam">{{ item.tenRam }}</span>
                  <span v-if="item.tenRam && (item.tenRom || item.tenMauSac)"> • </span>
                  <span v-if="item.tenRom">{{ item.tenRom }}</span>
                  <span v-if="item.tenRom && item.tenMauSac"> • </span>
                  <span v-if="item.tenMauSac">{{ item.tenMauSac }}</span>
                  <span v-if="!item.tenHang && !item.tenRam && !item.tenRom && !item.tenMauSac">N/A</span>
                </p>
      ...
      <aside v-if="saleMode === 'quick'" class="w-96 bg-white shadow-lg flex flex-col border-l border-slate-200 h-full overflow-hidden">
        ...
        <div v-if="saleMode === 'quick'" class="flex-1 p-4 space-y-4 overflow-y-auto hide-scroll">
          ...
          <div class="space-y-3">
            ...
            <!-- Cash Payment Fields - Chỉ hiển thị khi chọn tiền mặt -->
            <div v-if="currentInvoice.paymentMethod === 'cash'">
              <div class="flex justify-between items-center text-lg mt-3">
            ...
            <!-- Bank Transfer Info - Hiển thị khi chọn chuyển khoản -->
            <div v-if="currentInvoice.paymentMethod === 'bank_transfer'" class="mt-3 p-3 bg-green-50 border border-green-200 rounded-lg">
              <div class="text-sm text-green-800">
                <p class="font-semibold mb-1">💳 Thanh toán chuyển khoản</p>
                <p class="text-xs">Đơn hàng sẽ được xử lý sau khi xác nhận chuyển khoản thành công.</p>
              </div>
            </div>
          </div>
          ...
          <div class="space-y-3">
            <div class="flex items-center space-x-4">
              ...
              <label class="flex items-center space-x-2">
                <input type="radio" name="paymentMethod" value="bank_transfer" v-model="currentInvoice.paymentMethod" @change="resetCombinedPaymentFields" class="text-orange-600">
          ...
          <button
            ...
            :class="[
              ...
              currentInvoice.paymentMethod === 'vnpay'
                ...
                : currentInvoice.paymentMethod === 'bank_transfer'
                ? 'bg-green-500 hover:bg-green-600 disabled:bg-slate-300'
                : 'bg-orange-400 hover:bg-orange-600 disabled:bg-slate-300'
            ]"
          >
            {{ currentInvoice.paymentMethod === 'vnpay' ? 'THANH TOÁN VNPAY' : currentInvoice.paymentMethod === 'bank_transfer' ? 'THANH TOÁN CHUYỂN KHOẢN' : 'THANH TOÁN TIỀN MẶT' }}
          </button>
      ...
      <aside v-if="saleMode === 'delivery'" class="w-[700px] bg-white shadow-lg flex flex-col border-l border-slate-200 h-full overflow-hidden">
        <!-- Header -->
        <div class="px-3 py-1.5 border-b border-slate-200 bg-gradient-to-r from-orange-50 to-orange-100">
          <h2 class="text-sm font-bold text-slate-800">Thông tin giao hàng</h2>
        </div>
        ...
        <div class="flex-1 overflow-y-auto p-4 hide-scroll">
          ...
          <div class="grid grid-cols-1 lg:grid-cols-2 gap-4">
            ...
            <div class="space-y-3">
              <h4 class="text-base font-semibold text-slate-800 mb-3 border-b-2 border-orange-400 pb-1.5">Thông tin khách hàng</h4>

              <!-- Phần chọn khách hàng -->
              <div class="bg-slate-50 p-3 rounded-lg border border-slate-200">
                <label class="block text-xs font-medium text-slate-700 mb-1.5">Chọn khách hàng</label>
                <div class="flex space-x-2">
                  <div class="relative flex-1">
                    <input
                      type="text"
                      ...
                      @focus="showCustomerSuggestions = true"
                      @blur="hideCustomerSuggestions"
                      class="w-full pl-9 pr-3 py-2 text-sm border border-slate-300 rounded-lg focus:ring-2 focus:ring-orange-500 focus:border-transparent"
                    />
                    ...
                    <!-- Customer Suggestions Dropdown -->
                    <div v-if="showCustomerSuggestions && filteredCustomers.length > 0" class="absolute top-full left-0 right-0 mt-1 bg-white border border-slate-300 rounded-lg shadow-lg max-h-48 overflow-y-auto z-50">
                      <div
                        v-for="customer in filteredCustomers"
                        :key="customer.id"
                        @click="selectCustomer(customer)"
                      ...
                      >
                        <div class="font-medium text-sm text-slate-800">{{ customer.hoTen }}</div>
                        <div class="text-xs text-slate-500">{{ customer.soDienThoai }}</div>
                        <div class="text-xs text-slate-400">{{ customer.email || 'Không có email' }}</div>
                      </div>
                    </div>
                  </div>
                  <button
                    @click="openQuickCustomerModal"
                    class="w-9 h-9 bg-orange-400 hover:bg-orange-600 text-white rounded-lg transition-colors flex items-center justify-center flex-shrink-0"
                    title="Thêm khách hàng nhanh"
                  >
                    <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                      <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M12 6v6m0 0v6m0-6h6m-6 0H6"></path>
                ...
                <!-- Selected Customer Display -->
                <div v-if="currentInvoice.selectedCustomer" class="mt-2 p-2 bg-orange-50 border border-orange-200 rounded-lg">
                  <div class="flex items-center justify-between">
                    <div class="flex-1">
                      <div class="font-medium text-sm text-slate-800">{{ currentInvoice.selectedCustomer.hoTen }}</div>
                      <div class="text-xs text-slate-600">{{ currentInvoice.selectedCustomer.soDienThoai }}</div>
                      <div class="text-xs text-slate-500">{{ currentInvoice.selectedCustomer.email || 'Không có email' }}</div>
                    </div>
                    <button @click="removeCustomer" class="text-red-500 hover:text-red-700 ml-2">
                      <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
              ...
              <div v-if="currentInvoice.selectedCustomer">
                <label class="block text-xs font-medium text-slate-700 mb-1">Địa chỉ trước đó</label>
                <select v-model="selectedPreviousAddressId" @change="applyPreviousAddress" class="w-full px-2.5 py-1.5 text-sm border border-slate-300 rounded-lg focus:ring-2 focus:ring-orange-500 focus:border-transparent">
              ...
              <div>
                <label class="block text-xs font-medium text-slate-700 mb-1">Tên người nhận <span class="text-red-500">*</span></label>
                <input type="text" v-model="deliveryInfo.tenNguoiNhan" placeholder="Nhập tên người nhận" class="w-full px-2.5 py-1.5 text-sm border border-slate-300 rounded-lg focus:ring-2 focus:ring-orange-500 focus:border-transparent" />
              </div>

              <div>
                <label class="block text-xs font-medium text-slate-700 mb-1">Số điện thoại <span class="text-red-500">*</span></label>
                <input type="tel" v-model="deliveryInfo.soDienThoai" placeholder="Nhập số điện thoại người nhận" class="w-full px-2.5 py-1.5 text-sm border border-slate-300 rounded-lg focus:ring-2 focus:ring-orange-500 focus:border-transparent" />
              </div>

              <div>
                <label class="block text-xs font-medium text-slate-700 mb-1">Tỉnh/Thành phố <span class="text-red-500">*</span></label>
                <select v-model="deliveryInfo.idTinhThanhPho" @change="onProvinceChange" class="w-full px-2.5 py-1.5 text-sm border border-slate-300 rounded-lg focus:ring-2 focus:ring-orange-500 focus:border-transparent" :disabled="loadingProvinces">
              ...
              <div>
                <label class="block text-xs font-medium text-slate-700 mb-1">Phường/Xã <span class="text-red-500">*</span></label>
                <select v-model="deliveryInfo.idPhuongXa" @change="onWardChange" class="w-full px-2.5 py-1.5 text-sm border border-slate-300 rounded-lg focus:ring-2 focus:ring-orange-500 focus:border-transparent" :disabled="!deliveryInfo.idTinhThanhPho || loadingWards">
              ...
              <div>
                <label class="block text-xs font-medium text-slate-700 mb-1">Địa chỉ chi tiết (Số nhà, ngõ, đường) <span class="text-red-500">*</span></label>
                <textarea v-model="deliveryInfo.soNhaDuong" placeholder="Ví dụ: 123/45, ngõ 12, đường ABC" class="w-full px-2.5 py-1.5 text-sm border border-slate-300 rounded-lg focus:ring-2 focus:ring-orange-500 focus:border-transparent" rows="2"></textarea>
              </div>

              <div>
                <label class="block text-xs font-medium text-slate-700 mb-1">Ghi chú (tùy chọn)</label>
                <textarea v-model="deliveryInfo.ghiChu" placeholder="Ghi chú thêm cho bưu tá (tùy chọn)" class="w-full px-2.5 py-1.5 text-sm border border-slate-300 rounded-lg focus:ring-2 focus:ring-orange-500 focus:border-transparent" rows="2"></textarea>
              </div>
            </div>

            <!-- Bên phải: Thông tin thanh toán -->
            <div class="space-y-3">
              <h4 class="text-base font-semibold text-slate-800 mb-3 border-b-2 border-orange-400 pb-1.5">Thông tin thanh toán</h4>

              <!-- Phí vận chuyển -->
              <div class="bg-slate-50 p-3 rounded-lg border border-slate-200">
                <h4 class="text-xs font-semibold text-slate-900 mb-2">Phí vận chuyển</h4>
                <div class="flex justify-between items-center">
                  <span class="text-xs text-slate-700">Phí ship:</span>
                  <input
                    type="text"
                    v-model="formattedShippingFee"
                    @input="handleShippingFeeInput"
                    class="w-28 text-right font-bold text-sm px-2 py-1 border border-slate-300 rounded focus:ring-2 focus:ring-orange-500 focus:border-transparent"
                    placeholder="0"
                  />
                </div>
              </div>
              ...
              <!-- Tổng tiền -->
              <div class="bg-slate-50 p-3 rounded-lg border border-slate-200">
                <h4 class="text-xs font-semibold text-slate-900 mb-2">Tổng tiền</h4>
                <div class="space-y-1.5">
                  <div class="flex justify-between items-center text-xs">
                    <span class="text-slate-600">Tổng tiền hàng:</span>
                    <span class="font-medium text-slate-800">{{ formatCurrency(currentInvoice.subtotal) }}</span>
                  </div>
                  <div v-if="currentInvoice.discount > 0" class="flex justify-between items-center text-xs">
                    <span class="text-slate-600">Giảm giá:</span>
                  ...
                  <div class="flex justify-between items-center pt-1.5 border-t border-slate-200">
                    ...
                    <span class="text-base font-bold text-orange-600">{{ formatCurrency(currentInvoice.total) }}</span>
                  </div>
                  <div v-if="deliveryInfo.shippingFee > 0" class="flex justify-between items-center text-xs pt-1.5 border-t border-slate-200">
                    <span class="text-slate-600">Phí vận chuyển:</span>
                    <span class="font-medium text-slate-800">{{ formatCurrency(deliveryInfo.shippingFee) }}</span>
                  </div>
                  <div class="flex justify-between items-center pt-1.5 border-t-2 border-orange-400">
                    <span class="text-sm font-semibold text-slate-900">Tổng thanh toán:</span>
                    <span class="text-lg font-bold text-orange-600">{{ formatCurrency(currentInvoice.total + (deliveryInfo.shippingFee || 0)) }}</span>
                  </div>
                </div>
              </div>
              ...
              <div class="bg-slate-50 p-3 rounded-lg border border-slate-200">
                <div class="flex items-center justify-between mb-2">
                  <h4 class="text-xs font-semibold text-slate-900">Voucher giảm giá</h4>
                  <button @click="openVoucherModal" class="text-xs text-orange-600 hover:text-orange-700 font-medium">Chọn voucher</button>
                </div>
                <div v-if="currentInvoice.selectedVoucher" class="p-2 bg-green-50 border border-green-200 rounded-lg">
                  <div class="flex items-center justify-between">
                    <div>
                      <div class="text-xs font-medium text-green-800">{{ currentInvoice.selectedVoucher.tenPhieuGiamGia }}</div>
                      <div class="text-xs text-green-600">{{ currentInvoice.selectedVoucher.maPhieuGiamGia }}</div>
                    </div>
                    <button @click="removeVoucher" class="w-5 h-5 bg-red-500 hover:bg-red-600 rounded-full text-white text-xs flex items-center justify-center ml-2">×</button>
                  </div>
                </div>
                <div v-else-if="bestVoucher" class="p-2 bg-orange-50 border border-orange-200 rounded-lg">
                  <div class="flex items-center justify-between">
                    <div>
                      <div class="text-xs font-medium text-orange-800">💡 Voucher tốt nhất</div>
                      <div class="text-xs text-orange-600">{{ bestVoucher.tenPhieuGiamGia }}</div>
                    </div>
                    <button @click="applyBestVoucher" class="px-2 py-0.5 bg-orange-400 hover:bg-orange-600 text-white text-xs rounded">Áp dụng</button>
                  </div>
                </div>
                <div v-else class="text-xs text-slate-500 text-center py-1.5">
                  Chưa có voucher
                </div>
              </div>
              ...
              <!-- Payment Method Selection -->
              <div class="bg-slate-50 p-3 rounded-lg border border-slate-200">
                <h4 class="text-xs font-semibold text-slate-900 mb-2">Phương thức thanh toán</h4>
                <div class="flex items-center space-x-4 mb-3">
                  ...
                  <label class="flex items-center space-x-2">
                    <input type="radio" name="paymentMethodDelivery" value="bank_transfer" v-model="currentInvoice.paymentMethod" @change="resetCombinedPaymentFields" class="text-orange-600">
              ...
              <!-- Cash Payment Fields - Chỉ hiển thị khi chọn tiền mặt -->
              <div v-if="currentInvoice.paymentMethod === 'cash'" class="bg-slate-50 p-3 rounded-lg border border-slate-200">
                ...
                <!-- Change Amount -->
                <div class="flex justify-between items-center text-sm mt-2" v-if="currentInvoice.customerPaid > (currentInvoice.total + (deliveryInfo.shippingFee || 0))">
                  <span class="font-semibold text-green-600">Tiền thừa</span>
                  <span class="font-bold text-black">{{ formatCurrency(currentInvoice.customerPaid - (currentInvoice.total + (deliveryInfo.shippingFee || 0))) }}</span>
                </div>
              </div>
            </div>
          </div>
        ...
        <div class="p-4 border-t border-slate-200 bg-slate-50 flex-shrink-0">
          <div class="flex items-center justify-between">
            <button @click="saleMode = 'quick'" class="px-5 py-2 border border-slate-300 text-slate-700 rounded-lg hover:bg-slate-100 transition-colors text-sm font-medium">
              Hủy
            </button>
            <button @click="confirmDelivery" :class="[
              'px-6 py-2.5 text-white rounded-lg transition-colors font-semibold text-base shadow-lg',
              currentInvoice.paymentMethod === 'bank_transfer' 
                ? 'bg-green-500 hover:bg-green-600' 
                : 'bg-orange-400 hover:bg-orange-600'
            ]">
              {{ currentInvoice.paymentMethod === 'bank_transfer' ? 'THANH TOÁN CHUYỂN KHOẢN' : 'THANH TOÁN' }}
            </button>
          </div>
        </div>
      </aside>
            ...
            >
              ...
              <div class="p-3">
                ...
                <div class="flex flex-wrap gap-1 mb-2">
                  <span v-if="product.tenRam" class="px-2 py-0.5 bg-blue-100 text-blue-700 text-xs rounded-full">{{ product.tenRam }}</span>
                  <span v-if="product.tenRom" class="px-2 py-0.5 bg-green-100 text-green-700 text-xs rounded-full">{{ product.tenRom }}</span>
                  <span v-if="product.tenMauSac" class="px-2 py-0.5 bg-purple-100 text-purple-700 text-xs rounded-full">{{ product.tenMauSac }}</span>
                </div>
      ...
      <div class="bg-gradient-to-br from-purple-50 to-pink-50 border-2 border-purple-200 rounded-xl w-4/5 max-w-2xl max-h-[80vh] flex flex-col shadow-2xl">
        ...
        <div class="flex-1 overflow-y-auto p-6">
          <div class="grid grid-cols-1 gap-3">
            <div
              v-for="imei in filteredImeis.filter(i => i.trangThai === 1 || i.trangThai === '1')"
              :key="imei.id"
    ...
    <!-- QR Scanner Modal -->
    <div v-if="showQRScannerModal" class="fixed inset-0 bg-black/50 backdrop-blur-sm flex items-center justify-center z-50" @click="closeQRScanner">
      <div class="bg-white rounded-xl shadow-2xl p-6 max-w-md w-full mx-4" @click.stop>
        <div class="flex items-center justify-between mb-4">
          <h3 class="text-xl font-semibold text-gray-800">Quét QR IMEI</h3>
          <button
        ...
        <div class="relative bg-gray-900 rounded-lg overflow-hidden" style="aspect-ratio: 1;">
          <video
            id="qr-scanner-video"
            class="w-full h-full object-cover"
            playsinline
          ></video>
          <div v-if="!isScanningQR" class="absolute inset-0 flex items-center justify-center bg-gray-800">
            <p class="text-white">Đang khởi động camera...</p>
          </div>
        </div>
        
        <p class="text-sm text-gray-600 mt-4 text-center">
          Đưa mã QR IMEI vào khung hình để quét
        </p>
      </div>
    </div>
...
export default {
  ...
  setup() {
    ...
    const confirmTitle = ref('Xác nhận thanh toán')
    ...
    const deliveryInfo = ref({
      ...
      tinhThanh: '',
      ghiChu: '',
      shippingFee: 0 // Phí vận chuyển nhập tay
    })
    ...
    // WebSocket client for product status updates
    ...
    // Filtered IMEIs for search - only show active IMEIs (trangThai === 1)
    const filteredImeis = computed(() => {
      // First filter by status to ensure only active IMEIs
      const activeImeis = availableImeis.value.filter(imei => imei.trangThai === 1 || imei.trangThai === '1')
      
      // Then filter by search query if provided
      if (!imeiSearchQuery.value) return activeImeis
      return activeImeis.filter(imei =>
    ...
    const handleQRScanned = async (qrData) => {
      try {
        // Validate QR code - chỉ chấp nhận IMEI (số 15 chữ số), không phải URL
        const trimmedData = qrData.trim()
        
        // Kiểm tra nếu là URL thì bỏ qua
        if (trimmedData.startsWith('http://') || trimmedData.startsWith('https://') || trimmedData.startsWith('www.')) {
          toastRef.value?.warning('Cảnh báo', 'QR code này là một liên kết, không phải IMEI. Vui lòng quét mã IMEI của sản phẩm.')
          return
        }
        
        // Kiểm tra nếu không phải số hoặc quá ngắn/dài
        const imeiPattern = /^[0-9]{10,20}$/ // IMEI thường có 15 chữ số, nhưng cho phép 10-20 để linh hoạt
        if (!imeiPattern.test(trimmedData)) {
          toastRef.value?.warning('Cảnh báo', 'QR code không hợp lệ. IMEI phải là dãy số từ 10-20 chữ số.')
          return
        }
        
        // Tìm sản phẩm theo IMEI
        const response = await api.get(`/api/imei/search?imei=${encodeURIComponent(trimmedData)}`)
        
        if (response.data && response.data.chiTietSanPham) {
          const imeiData = response.data
          const chiTietSanPham = imeiData.chiTietSanPham
          
          // Kiểm tra IMEI có sẵn không
          if (imeiData.trangThai !== 1) {
            toastRef.value?.warning('Cảnh báo', 'IMEI này không khả dụng hoặc đã được sử dụng')
            return
          }
          
          // Tìm product trong danh sách products hoặc tạo từ chiTietSanPham
          let product = products.value.find(p => p.chiTietSanPhamId === chiTietSanPham.id)
          
          // Nếu không tìm thấy trong danh sách, tạo product object từ chiTietSanPham
          if (!product) {
            // Load product details từ API
            try {
              const productResponse = await api.get(`/api/san-pham-pos/${chiTietSanPham.id}`)
              if (productResponse.data) {
                product = productResponse.data
              }
            } catch (err) {
              console.warn('Could not load product details, creating from chiTietSanPham:', err)
              // Tạo product object từ chiTietSanPham
              product = {
                chiTietSanPhamId: chiTietSanPham.id,
                id: chiTietSanPham.idSp || chiTietSanPham.id,
                tenSanPham: chiTietSanPham.tenSanPham || 'Sản phẩm',
                gia: chiTietSanPham.giaBan ? parseFloat(chiTietSanPham.giaBan) : 0,
                soLuongTon: chiTietSanPham.soLuong || 0,
                tenHang: chiTietSanPham.tenHang || '',
                tenRam: chiTietSanPham.tenRam || '',
                tenRom: chiTietSanPham.tenRom || '',
                tenMauSac: chiTietSanPham.tenMauSac || '',
                hinhAnh: chiTietSanPham.hinhAnh || '',
                trangThai: chiTietSanPham.trangThai || 1,
                trangThaiSanPham: 1
              }
            }
          }
          ...
          if (product.trangThai !== undefined && product.trangThai !== 1) {
            toastRef.value?.warning('Cảnh báo', `"${product.tenSanPham}" đã ngừng hoạt động`)
            return
          }
          ...
          // Kiểm tra tồn kho
          if (!product.soLuongTon || product.soLuongTon <= 0) {
            toastRef.value?.warning('Cảnh báo', `"${product.tenSanPham}" đã hết hàng`)
            return
          }
          
          // Kiểm tra IMEI đã tồn tại trong giỏ hàng chưa
          const existingImeiInCart = currentInvoice.value.cartItems.find(item => {
            return item.selectedImeis && item.selectedImeis.includes(trimmedData)
          })
          
          if (existingImeiInCart) {
            toastRef.value?.warning('Cảnh báo', `Sản phẩm với IMEI ${trimmedData} đã tồn tại trong giỏ hàng`)
            return
          }
          
          // Tự động thêm vào giỏ hàng với IMEI đã quét
          const cartItemId = `${product.chiTietSanPhamId}_${Date.now()}_${Math.random()}`
          
          // Giảm stock
          adjustStock(product.chiTietSanPhamId, -1)
          
          // Reserve IMEI
          addReservedImeis([trimmedData])
          ...
          currentInvoice.value.cartItems.push({
            ...
            gia: product.gia,
            giaKhiThem: product.gia,
            quantity: 1,
            soLuongTon: product.soLuongTon - 1, // Đã giảm bởi adjustStock
            tenHang: product.tenHang,
            tenRam: product.tenRam,
            tenRom: product.tenRom,
            tenMauSac: product.tenMauSac,
            hinhAnh: product.hinhAnh,
            selectedImeis: [trimmedData], // IMEI đã quét
            priceChanged: false,
            addedAt: Date.now()
          })
          
          calculateTotals()
          toastRef.value?.success('Thành công', `Đã thêm "${product.tenSanPham}" với IMEI ${trimmedData} vào giỏ hàng`)
        } else {
          toastRef.value?.error('Lỗi', 'Không tìm thấy sản phẩm với IMEI này')
        }
      } catch (error) {
        console.error('Error searching product by IMEI:', error)
        if (error.response?.status === 404) {
          toastRef.value?.error('Lỗi', error.response?.data?.error || 'Không tìm thấy sản phẩm với IMEI này')
        } else if (error.response?.status === 500) {
          toastRef.value?.error('Lỗi', error.response?.data?.error || 'Lỗi server khi tìm kiếm IMEI')
        } else {
          toastRef.value?.error('Lỗi', 'Không thể tìm sản phẩm theo IMEI. Vui lòng thử lại.')
        }
      }
    }
    ...
    const addToCart = (product) => {
      ...
      // Validate product and chiTietSanPhamId
      if (!product || !product.chiTietSanPhamId) {
        console.error('Invalid product or chiTietSanPhamId:', product)
        toastRef.value?.error('Lỗi', 'Thông tin sản phẩm không hợp lệ')
        return
      }
    ...
    const loadAvailableImeis = async (chiTietSanPhamId) => {
      // Validate chiTietSanPhamId before making API call
      if (!chiTietSanPhamId || chiTietSanPhamId === 'undefined' || chiTietSanPhamId === undefined) {
        console.error('Invalid chiTietSanPhamId:', chiTietSanPhamId)
        availableImeis.value = []
        toastRef.value?.error('Lỗi', 'Không tìm thấy thông tin sản phẩm. Vui lòng thử lại.')
        return
      }
      ...
      try {
        ...
        console.log('📱 Raw IMEI response:', response.data)
        // Filter only available IMEIs (trangThai must be exactly 1, not null or 0)
        const raw = (response.data || []).filter(imei => {
          const isActive = imei.trangThai === 1 || imei.trangThai === '1'
          if (!isActive) {
            console.warn('⚠️ Filtered out inactive IMEI:', imei.imei, 'trangThai:', imei.trangThai)
          }
          return isActive
        })
        console.log('✅ Filtered active IMEIs:', raw.length)
        ...
        console.log('✅ Final available IMEIs (after excluding reserved):', availableImeis.value.length)
      } catch (error) {
        console.error('Error loading IMEIs:', error)
        availableImeis.value = []
        if (error.response?.status === 404) {
          toastRef.value?.error('Lỗi', 'Không tìm thấy IMEI cho sản phẩm này')
        } else {
    ...
    const loadAvailableImeisForExistingProduct = async (chiTietSanPhamId, excludeImeis = []) => {
      ...
      try {
        // Load available IMEIs for the specific product variant
        const response = await api.get(`/api/imei/chi-tiet/${chiTietSanPhamId}`)
        console.log('📱 Raw IMEI response (existing product):', response.data)
        // Filter only available IMEIs (trangThai must be exactly 1, not null or 0)
        let allImeis = (response.data || []).filter(imei => {
          const isActive = imei.trangThai === 1 || imei.trangThai === '1'
          if (!isActive) {
            console.warn('⚠️ Filtered out inactive IMEI:', imei.imei, 'trangThai:', imei.trangThai)
          }
        ...
        console.log('✅ Filtered active IMEIs (existing product):', allImeis.length)
        ...
        console.log('✅ Final available IMEIs (after excluding used/reserved):', availableImeis.value.length)
      } catch (error) {
    ...
    const confirmImeiSelection = async () => {
      ...
      const existingImeiInCart = currentInvoice.value.cartItems.find(item => {
        if (!item.selectedImeis || item.selectedImeis.length === 0) return false
        return newImeis.some(imei => item.selectedImeis.includes(imei))
      })
      
      if (existingImeiInCart) {
        const duplicateImei = newImeis.find(imei => existingImeiInCart.selectedImeis.includes(imei))
        toastRef.value?.warning('Cảnh báo', `Sản phẩm với IMEI ${duplicateImei} đã tồn tại trong giỏ hàng`)
        return
      }
    ...
    const createQuickCustomer = async () => {
      try {
        ...
        const hoTenPattern = /^[\p{L}\s]+$/u
        if (!hoTenPattern.test(quickCustomerForm.value.hoTen.trim())) {
          toastRef.value?.error('Lỗi', 'Họ tên chỉ được chứa chữ cái và khoảng trắng')
          return
        }

        // Validate số điện thoại: bắt đầu bằng 0 và có 10 chữ số
        const phonePattern = /^(0[3|5|7|8|9])+([0-9]{8})$/
        if (!phonePattern.test(quickCustomerForm.value.soDienThoai.trim())) {
          toastRef.value?.error('Lỗi', 'Số điện thoại phải bắt đầu bằng 0 và có 10 chữ số (ví dụ: 0123456789)')
          return
        }

        // Validate email: nếu có thì phải hợp lệ, nếu không có thì tạo email mặc định
        let email = quickCustomerForm.value.email?.trim() || ''
        if (email) {
          const emailPattern = /^[^\s@]+@[^\s@]+\.[^\s@]+$/
          if (!emailPattern.test(email)) {
            toastRef.value?.error('Lỗi', 'Email không hợp lệ')
            return
          }
        } else {
          // Tạo email mặc định từ số điện thoại nếu không có email
          email = `customer_${quickCustomerForm.value.soDienThoai.trim()}@pos.local`
        }

        // Chuẩn bị dữ liệu gửi lên server
        const customerData = {
          hoTen: quickCustomerForm.value.hoTen.trim(),
          soDienThoai: quickCustomerForm.value.soDienThoai.trim(),
          email: email,
          // Chỉ gửi diaChi nếu có giá trị
          ...(quickCustomerForm.value.diaChi?.trim() && { diaChi: quickCustomerForm.value.diaChi.trim() })
        }
        ...
        let errorMessage = 'Không thể tạo khách hàng mới'
    ...
    // Xử lý khi sản phẩm bị vô hiệu hóa
    const handleProductDeactivated = (message) => {
      const chiTietSanPhamId = message.chiTietSanPhamId
      const tenSanPham = message.tenSanPham || 'Sản phẩm'
      
      // Xóa sản phẩm khỏi tất cả giỏ hàng trong tất cả invoices
      invoices.value.forEach(invoice => {
        if (invoice.cartItems) {
          const itemsToRemove = invoice.cartItems.filter(item => 
            item.chiTietSanPhamId === chiTietSanPhamId
          )
          
          itemsToRemove.forEach(item => {
            const index = invoice.cartItems.findIndex(i => 
              (i.cartItemId || i.chiTietSanPhamId) === (item.cartItemId || item.chiTietSanPhamId)
            )
            if (index > -1) {
              const removedItem = invoice.cartItems[index]
              // Return reserved stock
              const qty = removedItem.quantity || 1
              const imeis = removedItem.selectedImeis || []
              if (qty > 0) adjustStock(removedItem.chiTietSanPhamId, +qty)
              if (imeis.length > 0) releaseReservedImeis(imeis)
              invoice.cartItems.splice(index, 1)
            }
          })
          
          // Recalculate totals if this is the current invoice
          if (invoice.id === activeInvoiceId.value) {
            calculateTotals()
          }
        }
      })
      ...
      // Hiển thị thông báo
      if (toastRef.value) {
        toastRef.value.warning('Sản phẩm đã ngừng hoạt động', 
          `Sản phẩm "${tenSanPham}" đã được vô hiệu hóa và đã được xóa khỏi giỏ hàng`)
      }
    }
    ...
    const revalidateCurrentVoucher = async (openChooserOnInvalid = false) => {
      ...
      if (!stillAvailable) {
        ...
        if (bestVoucher.value && bestVoucher.value.tenPhieuGiamGia) {
    ...
    // Formatted shipping fee
    const formattedShippingFee = computed({
      get() {
        return deliveryInfo.value.shippingFee ? deliveryInfo.value.shippingFee.toLocaleString('vi-VN') : ''
      },
      set(value) {
        // This will be handled by handleShippingFeeInput
      }
    })

    const handleShippingFeeInput = (event) => {
      const value = event.target.value
      // Remove all non-numeric characters
      const numericValue = value.replace(/[^\d]/g, '')

      if (numericValue === '') {
        deliveryInfo.value.shippingFee = 0
      } else {
        deliveryInfo.value.shippingFee = parseInt(numericValue)
      }
    }
    ...
    // Sales methods - Define performNormalSale first
    const performNormalSale = async () => {
      ...
      try {
        ...
        // Lấy nhanVienId từ auth store
        const authStore = useAuthStore()
        const nhanVienId = authStore.user?.idNguoiDung || authStore.user?.id || null
        console.log('📝 Creating order with nhanVienId:', nhanVienId)

        // Validate totals
        const tongTien = parseFloat(currentInvoice.value.subtotal || 0)
        const tongTienSauGiam = parseFloat(currentInvoice.value.total || 0)
        
        if (isNaN(tongTien) || tongTien < 0) {
          throw new Error('Tổng tiền không hợp lệ')
        }
        if (isNaN(tongTienSauGiam) || tongTienSauGiam < 0) {
          throw new Error('Tổng tiền sau giảm không hợp lệ')
        }
        ...
        const orderData = {
          hoaDon: {
            ...
            tongTien: tongTien, // Sử dụng subtotal (tổng tiền hàng trước giảm giá)
            tongTienSauGiam: tongTienSauGiam, // Sử dụng total (tổng tiền sau giảm giá)
            loaiHoaDon: 'NORMAL',
            ghiChu: currentInvoice.value.orderNote || null,
            ...
            diaChi: currentInvoice.value.selectedCustomer?.diaChi || null, // Thêm địa chỉ nếu có
            phuongThucThanhToan: currentInvoice.value.paymentMethod === 'bank_transfer' ? 'BANK_TRANSFER' : 'CASH',
          },
          chiTietHoaDon: currentInvoice.value.cartItems.map(item => {
            // Sử dụng giaKhiThem (giá tại thời điểm thêm vào) nếu có, nếu không thì dùng gia
            const price = parseFloat(item.giaKhiThem || item.gia || 0)
            const quantity = parseInt(item.quantity || 1)
            const thanhTien = price * quantity
            
            // Validate
            if (!item.chiTietSanPhamId) {
              throw new Error(`Sản phẩm "${item.tenSanPham || 'N/A'}" không có ID chi tiết sản phẩm`)
            }
            if (isNaN(price) || price <= 0) {
              throw new Error(`Sản phẩm "${item.tenSanPham || 'N/A'}" có giá không hợp lệ: ${price}`)
            }
            if (isNaN(quantity) || quantity <= 0) {
              throw new Error(`Sản phẩm "${item.tenSanPham || 'N/A'}" có số lượng không hợp lệ: ${quantity}`)
            }
        ...
        // If VNPay payment or Bank Transfer, create payment URL first
        if (currentInvoice.value.paymentMethod === 'vnpay' || currentInvoice.value.paymentMethod === 'bank_transfer') {
          try {
            const paymentData = {
            ...
            const errorMessage = currentInvoice.value.paymentMethod === 'bank_transfer' 
              ? 'Không thể tạo thanh toán chuyển khoản: ' 
              : 'Không thể tạo thanh toán VNPay: '
            toastRef.value?.error('Lỗi', errorMessage + (paymentError.response?.data?.message || paymentError.message))
            return
          }
    ...
    const normalSale = async () => {
      ...
      const paymentMethodText = currentInvoice.value.paymentMethod === 'vnpay' ? 'VNPay' : 
                                currentInvoice.value.paymentMethod === 'combined' ? 'Kết hợp' :
                                currentInvoice.value.paymentMethod === 'bank_transfer' ? 'Chuyển khoản' : 'Tiền mặt'
      const totalAmount = formatCurrency(currentInvoice.value.total)
      confirmTitle.value = 'Xác nhận thanh toán'
      confirmMessage.value = `Bạn có chắc chắn muốn thanh toán đơn hàng này?\n\nPhương thức: ${paymentMethodText}\nTổng tiền: ${totalAmount}`
      pendingPaymentAction.value = performNormalSale
    ...
    const switchToDeliveryMode = async () => {
      ...
      currentInvoice.value.paymentMethod = 'cash' // Mặc định là tiền mặt
      resetDeliveryInfo()
    ...
    const switchToQuickMode = () => {
      saleMode.value = 'quick'
      currentInvoice.value.paymentMethod = 'cash' // Chỉ cho phép tiền mặt cho bán nhanh
    }
    ...
    const confirmDelivery = async () => {
      ...
      // Hiển thị confirm modal
      const totalAmount = formatCurrency(currentInvoice.value.total + (deliveryInfo.value.shippingFee || 0))
      const paymentMethodText = currentInvoice.value.paymentMethod === 'bank_transfer' ? 'Chuyển khoản' : 'Tiền mặt'
      confirmTitle.value = 'Xác nhận thanh toán giao hàng'
      confirmMessage.value = `Bạn có chắc chắn muốn thanh toán đơn hàng giao hàng này?\n\nPhương thức: ${paymentMethodText}\nTổng tiền: ${totalAmount}\nPhí vận chuyển: ${formatCurrency(deliveryInfo.value.shippingFee || 0)}`
      pendingPaymentAction.value = performConfirmDelivery
      showConfirmModal.value = true
    }
    ...
    const performConfirmDelivery = async () => {
      ...
      // Tạo địa chỉ đầy đủ
      const addressParts = []
      if (deliveryInfo.value.soNhaDuong) addressParts.push(deliveryInfo.value.soNhaDuong)
      if (deliveryInfo.value.phuongXa) addressParts.push(deliveryInfo.value.phuongXa)
      if (deliveryInfo.value.quanHuyen) addressParts.push(deliveryInfo.value.quanHuyen)
      if (deliveryInfo.value.tinhThanhPho) addressParts.push(deliveryInfo.value.tinhThanhPho)
      const fullAddress = addressParts.join(', ').trim()

      // Validate địa chỉ không được rỗng
      if (!fullAddress || fullAddress.length === 0) {
        toastRef.value?.error('Lỗi', 'Địa chỉ không hợp lệ. Vui lòng kiểm tra lại thông tin địa chỉ.')
        return
      }

      // Tính tổng tiền sau giảm giá (bao gồm phí ship)
      let finalTotal = currentInvoice.value.total + (deliveryInfo.value.shippingFee || 0)
      
      // Validate số tiền khách đưa - chỉ yêu cầu khi thanh toán tiền mặt
      if (currentInvoice.value.paymentMethod === 'cash') {
        if (!currentInvoice.value.customerPaid || currentInvoice.value.customerPaid < finalTotal) {
          toastRef.value?.error('Lỗi', `Số tiền khách đưa phải lớn hơn hoặc bằng ${formatCurrency(finalTotal)}`)
          loading.value = false
          return
        }
      }
      ...
      try {
        const orderData = {
          hoaDon: {
            ...
            tongTienSauGiam: finalTotal, // Tổng tiền sau giảm giá (bao gồm phí ship)
            loaiHoaDon: 'DELIVERY',
            phuongThucGiaoHang: 'Giao hàng',
            diaChi: fullAddress,
            ...
            phieuGiamGiaId: currentInvoice.value.selectedVoucher ? currentInvoice.value.selectedVoucher.id : null,
            phuongThucThanhToan: currentInvoice.value.paymentMethod === 'bank_transfer' ? 'BANK_TRANSFER' : 'CASH',
            phiVanChuyen: deliveryInfo.value.shippingFee || 0,
          },
          chiTietHoaDon: currentInvoice.value.cartItems.map(item => {
            // Sử dụng giaKhiThem (giá tại thời điểm thêm vào) nếu có, nếu không thì dùng gia
            const price = parseFloat(item.giaKhiThem || item.gia || 0)
        ...
        if (currentInvoice.value.paymentMethod === 'vnpay' || currentInvoice.value.paymentMethod === 'bank_transfer') {
          try {
            const paymentData = {
              amount: finalTotal,
              orderInfo: `Thanh toan don hang giao hang - ${deliveryInfo.value.tenNguoiNhan || currentInvoice.value.selectedCustomer?.hoTen || 'Khach hang'}`,
              order: orderData
            }
          ...
          try {
            // Tính lại VNPay amount nếu có COD fee
            let vnpayAmount = currentInvoice.value.vnpayAmount || 0
            if (deliveryInfo.value.codEnabled && deliveryInfo.value.codFee > 0) {
              // Nếu có COD, cần điều chỉnh VNPay amount để bao gồm phần COD tương ứng
              // Giả sử COD được thanh toán bằng tiền mặt, VNPay chỉ thanh toán phần đơn hàng
              vnpayAmount = currentInvoice.value.vnpayAmount
            }
        ...
        // Hiển thị thông báo lỗi chi tiết từ server
        let errorMessage = 'Có lỗi xảy ra khi tạo đơn giao hàng'
        if (error.response?.data) {
          const errorData = error.response.data
          if (typeof errorData === 'object') {
    ...
    // Lifecycle
    // Hàm xử lý khi IMEI bị vô hiệu hóa
    const handleImeiDeactivated = async (eventData) => {
      const { imei, imeiId, chiTietSanPhamId } = eventData
      
      // Kiểm tra xem IMEI có đang được sử dụng trong cartItems không
      const affectedItems = []
      currentInvoice.value.cartItems.forEach((item, itemIndex) => {
        if (item.selectedImeis && item.selectedImeis.includes(imei)) {
          affectedItems.push({ item, itemIndex, chiTietSanPhamId: item.chiTietSanPhamId })
        }
      })
      
      if (affectedItems.length > 0) {
        // Lưu thông tin item cần mở modal (trước khi xóa IMEI)
        let itemToReopen = null
        
        // IMEI đang được sử dụng, xóa nó khỏi cartItems
        // Xử lý từ cuối lên để tránh lỗi index khi splice
        for (let i = affectedItems.length - 1; i >= 0; i--) {
          const { item, itemIndex, chiTietSanPhamId } = affectedItems[i]
          // Xóa IMEI bị vô hiệu khỏi selectedImeis
          const imeiIndex = item.selectedImeis.indexOf(imei)
          if (imeiIndex > -1) {
            item.selectedImeis.splice(imeiIndex, 1)
            // Giảm quantity nếu cần
            if (item.selectedImeis.length < item.quantity) {
              item.quantity = item.selectedImeis.length
            }
            // Nếu không còn IMEI nào, xóa item khỏi cart
            if (item.selectedImeis.length === 0) {
              // Tìm lại index hiện tại của item (có thể đã thay đổi sau các lần splice trước)
              const currentIndex = currentInvoice.value.cartItems.findIndex(
                cartItem => cartItem.cartItemId === item.cartItemId
              )
              if (currentIndex > -1) {
                currentInvoice.value.cartItems.splice(currentIndex, 1)
                // Tăng lại stock
                adjustStock(item.chiTietSanPhamId, +item.quantity)
              }
              // Release reserved IMEIs
              releaseReservedImeis([imei])
            } else {
              // Release IMEI bị vô hiệu
              releaseReservedImeis([imei])
              
              // Lưu item này để mở modal (nếu chưa có item nào được lưu)
              if (!itemToReopen) {
                // Kiểm tra lại xem item có còn trong cart không
                const stillInCart = currentInvoice.value.cartItems.find(
                  cartItem => cartItem.cartItemId === item.cartItemId
                )
                if (stillInCart && stillInCart.quantity > stillInCart.selectedImeis.length) {
                  itemToReopen = {
                    item: stillInCart,
        ...
        // Hiển thị thông báo và mở lại modal chọn IMEI
        toastRef.value?.warning(
          'IMEI đã bị vô hiệu hóa',
          `IMEI ${imei} vừa được ngừng hoạt động. Vui lòng chọn IMEI khác.`
        )
        
        // Mở modal chọn IMEI nếu có item cần bổ sung IMEI
        if (itemToReopen) {
          // Kiểm tra lại xem item có còn trong cart không (sau khi đã xóa IMEI)
          const stillInCart = currentInvoice.value.cartItems.find(
            cartItem => cartItem.cartItemId === itemToReopen.item.cartItemId
          )
          
          if (stillInCart && stillInCart.quantity > stillInCart.selectedImeis.length) {
            // Tìm product tương ứng
            const product = products.value.find(p => p.chiTietSanPhamId === itemToReopen.chiTietSanPhamId)
            if (product && product.chiTietSanPhamId) {
              const missingImeiCount = stillInCart.quantity - stillInCart.selectedImeis.length
              console.log('🔄 Mở modal chọn IMEI:', {
                product: product.tenSanPham,
                chiTietSanPhamId: itemToReopen.chiTietSanPhamId,
                missingImeiCount: missingImeiCount,
                currentQuantity: stillInCart.quantity,
                currentImeis: stillInCart.selectedImeis.length
              })
              
              selectedProduct.value = product
              quantityToAdd.value = missingImeiCount
              selectedImeis.value = []
              
              // Load available IMEIs
              await loadAvailableImeis(product.chiTietSanPhamId)
              ...
              console.log('✅ Đã mở modal chọn IMEI cho sản phẩm:', product.tenSanPham, 'Cần thêm:', quantityToAdd.value, 'IMEI')
            } else {
              console.warn('⚠️ Không tìm thấy product với chiTietSanPhamId:', itemToReopen.chiTietSanPhamId)
            }
          } else {
            console.log('ℹ️ Item không còn trong cart hoặc đã đủ IMEI:', {
              stillInCart: !!stillInCart,
              quantity: stillInCart?.quantity,
              selectedImeisLength: stillInCart?.selectedImeis?.length
            })
          }
        } else {
          console.log('ℹ️ Không có item nào cần mở modal (có thể đã bị xóa hoàn toàn)')
        }
      }
    }

    // Lưu reference của event handlers để có thể remove sau
    let handleImeiDeactivatedEvent = null
    ...
    onMounted(async () => {
      ...
      try {
        ...
        stompClient.connect({}, () => {
          console.log('WebSocket connected for product status updates')
          // Subscribe to product status updates
          stompClient.subscribe('/topic/product-status', (message) => {
            try {
              const data = JSON.parse(message.body)
              console.log('Received product status update:', data)
              if (data.type === 'PRODUCT_DEACTIVATED' || data.type === 'PRODUCT_DETAIL_DEACTIVATED') {
                handleProductDeactivated(data)
              }
            } catch (error) {
              console.error('Error processing product status message:', error)
            }
          })
        }, (error) => {
      ...
      // Lắng nghe event khi IMEI bị vô hiệu hóa
      handleImeiDeactivatedEvent = (event) => {
        handleImeiDeactivated(event.detail)
      }
      
      handleStorageEvent = (event) => {
        if (event.key === 'imei-deactivated' && event.newValue) {
          try {
            const data = JSON.parse(event.newValue)
            handleImeiDeactivated(data)
          } catch (error) {
            console.error('Error parsing IMEI deactivated data:', error)
          }
        }
      }
      
      window.addEventListener('imei-deactivated', handleImeiDeactivatedEvent)
      window.addEventListener('storage', handleStorageEvent)
      
      // Kiểm tra localStorage khi mount (để xử lý trường hợp event đã xảy ra trước khi mount)
      const storedData = localStorage.getItem('imei-deactivated')
      if (storedData) {
        try {
          const data = JSON.parse(storedData)
          // Chỉ xử lý nếu event xảy ra trong vòng 5 giây gần đây
          if (Date.now() - data.timestamp < 5000) {
            handleImeiDeactivated(data)
          }
          // Xóa sau khi xử lý
          localStorage.removeItem('imei-deactivated')
        } catch (error) {
          console.error('Error parsing stored IMEI deactivated data:', error)
        }
      }
    ...
    onBeforeUnmount(() => {
      ...
      if (handleImeiDeactivatedEvent) {
        window.removeEventListener('imei-deactivated', handleImeiDeactivatedEvent)
      }
