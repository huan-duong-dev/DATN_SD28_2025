            ...
            <tbody>
              <tr v-for="(manHinh, index) in paginatedManHinhs" :key="manHinh.id">
                  ...
                  <div class="customer-info">
                    ...
                    <div class="customer-details">
                      ...
                      <div class="customer-email">
                        {{ manHinh.congNghe || 'Chưa có công nghệ' }}
                      </div>
                    </div>
                  </div>
                </td>
                ...
                <td>
                  <span
                    class="status-badge"
                    :class="manHinh.trangThai === 1 ? 'badge-active' : 'badge-inactive'"
                  >
                ...
                <td>
                  <div class="action-buttons">
                    ...
                    <div class="toggle-container">
                      <label
                        class="toggle-switch"
                        :title="manHinh.trangThai === 1 ? 'Gạt để vô hiệu hóa' : 'Gạt để kích hoạt'"
                      >
                        <input
                          type="checkbox"
      ...
      <div class="enhanced-modal">
          ...
          <div class="form-fields">
            <div class="field-group">
              ...
              <div class="input-with-button">
                <input
                  ...
                  class="form-input"
                  @blur="formData.maManHinh = normalizeCode(formData.maManHinh)"
                  required
                />
            ...
            <div class="field-group">
              ...
              <input
                ...
                class="form-input"
                @blur="formData.kichThuoc = normalizeName(formData.kichThuoc)"
                required
              />
        ...
        <div class="modal-footer">
          ...
          <button @click="handleFormSubmit" class="btn-save">
            ...
            {{ editingManHinh ? 'Cập nhật' : 'Thêm mới' }}
          </button>
        </div>
      </div>
    </div>
...
const viewingManHinh = ref<ManHinh | null>(null)
  ...
  if (typeFilter.value) {
    filtered = filtered.filter((manHinh) => {
      ...
      if (type === 'lcd') {
        ...
        return (
          !congNghe.includes('lcd') &&
          !congNghe.includes('oled') &&
          !congNghe.includes('amoled') &&
          !congNghe.includes('ips') &&
          !kieuManHinh.includes('lcd') &&
          !kieuManHinh.includes('oled') &&
          !kieuManHinh.includes('amoled') &&
          !kieuManHinh.includes('ips')
        )
      }
      return false
    })
...
const isAllSelected = computed(() => {
  return (
    paginatedManHinhs.value.length > 0 &&
...
function showInactiveManHinhs() {
  ...
  toastRef.value?.info(
    'Thông báo',
    'Đang hiển thị màn hình ngừng hoạt động. Gạt toggle để kích hoạt lại!',
  )
}
...
function viewManHinh(manHinh: ManHinh) {
  viewingManHinh.value = manHinh
  showViewModal.value = true
}

const viewFields = [
  { key: 'maManHinh', label: 'Mã màn hình', type: 'text' },
  { key: 'kichThuoc', label: 'Kích thước', type: 'text' },
  { key: 'congNghe', label: 'Công nghệ', type: 'text' },
  { key: 'doPhanGiai', label: 'Độ phân giải', type: 'text' },
  { key: 'tanSoQuet', label: 'Tần số quét', type: 'text' },
  { key: 'kieuManHinh', label: 'Kiểu màn hình', type: 'text' },
...
function exportToExcel() {
  try {
    ...
    const manHinhsToExport =
      selectedManHinhsComputed.value.length > 0 ? selectedManHinhsComputed.value : manHinhs.value
    ...
    const excelData = manHinhsToExport.map((manHinh, index) => ({
      ...
      'Ngày Cập Nhật': manHinh.ngayCapNhat
        ? new Date(manHinh.ngayCapNhat).toLocaleDateString('vi-VN')
        : '',
    }))
...
async function handleFormSubmit() {
  ...
  if (formData.value.maManHinh) {
    formData.value.maManHinh = normalizeCode(formData.value.maManHinh)
  }
  if (formData.value.kichThuoc) {
    formData.value.kichThuoc = normalizeName(formData.value.kichThuoc)
  }
  ...
  const validation = await validateAttributeForm(
    formData.value,
    { code: 'maManHinh', name: 'kichThuoc' },
    { endpoint: '/api/man-hinh', nameLabel: 'màn hình', editingId: editingManHinh.value?.id },
    toastRef.value,
  )
