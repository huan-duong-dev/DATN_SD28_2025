...
const filteredSanPhams = computed(() => {
  ...
  return sanPhams.value.filter((sp) => {
    const matchK =
      !k || sp.tenSanPham.toLowerCase().includes(k) || sp.maSanPham?.toLowerCase().includes(k)
    const matchHang =
      !selectedHang.value ||
      sp.tenHang === hangs.value.find((h) => h.id === selectedHang.value)?.ten
    const matchHdh =
      !selectedHeDieuHanh.value ||
      sp.tenHeDieuHanh ===
        heDieuHanhs.value.find((h) => h.id === selectedHeDieuHanh.value)?.tenHeDieuHanh
    const matchMh =
      !selectedManHinh.value ||
      sp.tenManHinh === manHinhs.value.find((m) => m.id === selectedManHinh.value)?.kichThuoc
    const matchPin =
      !selectedPin.value ||
      sp.tenPin === pins.value.find((p) => p.id === selectedPin.value)?.dungLuongPin
    const matchStatus =
...
function openViewDetailModal(sanPham: SanPham) {
  selectedProduct.value = sanPham
  showViewModal.value = true
}

const productDetailFields = computed(() => [
  { key: 'maSanPham', label: 'Mã sản phẩm', type: 'text' },
  { key: 'tenSanPham', label: 'Tên sản phẩm', type: 'text' },
  { key: 'tenDanhMuc', label: 'Danh mục', type: 'text' },
  { key: 'tenHang', label: 'Hãng', type: 'text' },
  { key: 'tenHeDieuHanh', label: 'Hệ điều hành', type: 'text' },
  { key: 'tenManHinh', label: 'Màn hình', type: 'text' },
  { key: 'tenCameraTruoc', label: 'Camera trước', type: 'text' },
  { key: 'tenCameraSau', label: 'Camera sau', type: 'text' },
  { key: 'tenChip', label: 'Chip', type: 'text' },
  { key: 'tenCpu', label: 'CPU', type: 'text' },
  { key: 'tenGpu', label: 'GPU', type: 'text' },
  { key: 'tenSim', label: 'SIM', type: 'text' },
  { key: 'tenPin', label: 'Pin', type: 'text' },
  { key: 'moTa', label: 'Mô tả', type: 'text' },
  { key: 'thietKe', label: 'Thiết kế', type: 'text' },
  { key: 'kichThuoc', label: 'Kích thước', type: 'text' },
...
async function toggleProductStatus(sanPham: SanPham) {
  ...
  try {
    ...
    showToastMessage(
      'Không thể cập nhật trạng thái sản phẩm: ' + (error.response?.data?.message || error.message),
      'error',
    )
  } finally {
    ...
    for (let i = 0; i < sanPhams.value.length; i++) {
      ...
      try {
        ...
        if (allChiTietData && Array.isArray(allChiTietData)) {
          chiTietData = allChiTietData.filter(
            (ct: any) =>
        ...
        if (product.giaBan || product.gia) {
          ...
          console.log(
            `🔍 No price data found for ${product.tenSanPham}, creating mock data for testing`,
          )
...
async function updateStatusForZeroQuantityProducts() {
  const productsToUpdate = sanPhams.value.filter(
    (sp) => (sp.tongImei || 0) === 0 && sp.trangThai === 1,
  )
...
function getImportPriceText(sanPham: any) {
  ...
  if (sanPham.giaNhapMin && sanPham.giaNhapMax) {
    console.log('🔍 Using calculated import price range:', {
      min: sanPham.giaNhapMin,
      max: sanPham.giaNhapMax,
    })
    if (sanPham.giaNhapMin === sanPham.giaNhapMax) {
...
function getSalePriceText(sanPham: any) {
  ...
  if (sanPham.giaBanMin && sanPham.giaBanMax) {
    console.log('🔍 Using calculated price range:', {
      min: sanPham.giaBanMin,
      max: sanPham.giaBanMax,
    })
    if (sanPham.giaBanMin === sanPham.giaBanMax) {
...
const allCurrentPageSelected = computed(() => {
  return (
    paginatedSanPhams.value.length > 0 &&
...
function exportToExcel(products: SanPham[]) {
  try {
    ...
    link.setAttribute(
      'download',
      `danh_sach_san_pham_${new Date().toISOString().split('T')[0]}.csv`,
    )
      ...
      <div class="filter-section">
        <div class="filter-header">
          ...
          <div class="filter-actions">
            ...
            <button class="btn-clear-filters" @click="resetAllFilters">Xóa bộ lọc</button>
          </div>
        </div>
        <div class="filter-row">
          ...
          <div class="filter-group">
            ...
            <select v-model.number="selectedHeDieuHanh" class="filter-select">
              ...
              <option v-for="o in heDieuHanhs" :key="o.id" :value="o.id">
                {{ o.tenHeDieuHanh }}
              </option>
            </select>
          </div>
          <div class="filter-group">
            ...
            <select v-model.number="selectedManHinh" class="filter-select">
              ...
              <option v-for="m in manHinhs" :key="m.id" :value="m.id">
                {{ m.kichThuoc }} {{ m.doPhanGiai ? '- ' + m.doPhanGiai : '' }}
              </option>
            </select>
          </div>
          <div class="filter-group">
      ...
      <div class="main-content">
          ...
          <table>
            ...
            <tbody>
              <tr v-for="(sp, idx) in paginatedSanPhams" :key="sp.id">
                ...
                <td :class="{ 'attribute-not-updated': getHangName(sp) === 'Chưa cập nhật' }">
                  {{ getHangName(sp) }}
                </td>
                <td :class="{ 'attribute-not-updated': getHeDieuHanhName(sp) === 'Chưa cập nhật' }">
                  {{ getHeDieuHanhName(sp) }}
                </td>
                <td :class="{ 'attribute-not-updated': getManHinhName(sp) === 'Chưa cập nhật' }">
                  {{ getManHinhName(sp) }}
                </td>
                <td :class="{ 'attribute-not-updated': getPinName(sp) === 'Chưa cập nhật' }">
                  {{ getPinName(sp) }}
                </td>
                <td>{{ sp.tongImei || 0 }}</td>
                <td class="price-cell">
                  <div class="price-container">
                    <div
                      v-if="
                        getImportPriceText(sp) === 'Chưa có giá' &&
                        getSalePriceText(sp) === 'Chưa có giá'
                      "
                      class="no-price"
                    >
                      Chưa có giá
                ...
                <td>
                  <span
                    class="status-badge"
                    :class="(sp.trangThai || 0) === 1 ? 'badge-active' : 'badge-inactive'"
                  >
                    {{ getStatusText(sp) }}
                  </span>
                </td>
                <td>
                  <div class="action-buttons">
                    <button
                      class="btn-info"
                      @click="openViewDetailModal(sp)"
                      title="Thông tin chi tiết"
                    >
                      <font-awesome-icon icon="info-circle" />
                    </button>
                    <button class="btn-view" @click="viewProduct(sp)" title="Xem chi tiết đầy đủ">
        ...
        <div class="pagination-section" v-if="totalPages > 1">
          <div class="pagination-info">
            <span
              >Hiển thị {{ startItem }} - {{ endItem }} / {{ filteredSanPhams.length }} mục</span
            >
          </div>
