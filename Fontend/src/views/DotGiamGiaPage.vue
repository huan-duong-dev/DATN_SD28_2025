...
async function toggleStatus(id: number) {
  ...
  if (!item) {
    toastRef.value?.error('Lỗi', 'Không tìm thấy đợt giảm giá')
    return
  }
  ...
  if (startDate > endDate) {
    toastRef.value?.error('Thời gian không hợp lệ', 'Ngày bắt đầu không thể lớn hơn ngày kết thúc. Vui lòng kiểm tra lại thông tin đợt giảm giá.')
    return
  }
  ...
  if (item.trangThai === 0) {
    ...
    if (isNotStarted(item)) {
      toastRef.value?.warning('Chưa thể kích hoạt', `Đợt giảm giá chưa đến thời gian hiệu lực. Ngày bắt đầu: ${formatDate(item.ngayBatDau)}`)
      return
    }

    // Kiểm tra nếu thời gian hiện tại không nằm trong khoảng thời gian hiệu lực
    if (now < startDate || now > endDate) {
      toastRef.value?.error('Thời gian không hợp lệ', `Không thể kích hoạt đợt giảm giá. Thời gian hiệu lực: ${formatDate(item.ngayBatDau)} - ${formatDate(item.ngayKetThuc)}`)
      return
    }
  }
