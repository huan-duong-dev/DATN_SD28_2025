  ...
  try {
    ...
    if (data) {
      ...
      trackingData.value = {
        ...
        // Đảm bảo lichSuThanhToan luôn là array
        lichSuThanhToan: Array.isArray(data.lichSuThanhToan) ? data.lichSuThanhToan : (data.lichSuThanhToan ? [data.lichSuThanhToan] : [])
      }
      ...
      console.log('🔄 lichSuThanhToan:', trackingData.value?.lichSuThanhToan)
      console.log('🔄 lichSuThanhToan length:', trackingData.value?.lichSuThanhToan?.length)
...
async function searchTracking() {
  ...
  try {
    ...
    if (data) {
      ...
      console.log('✅ lichSuTrangThai length:', trackingData.value?.lichSuTrangThai?.length)
      console.log('✅ lichSuThanhToan:', trackingData.value?.lichSuThanhToan)
      console.log('✅ lichSuThanhToan type:', Array.isArray(trackingData.value?.lichSuThanhToan))
      console.log('✅ lichSuThanhToan length:', trackingData.value?.lichSuThanhToan?.length)
...
function isStepClickable(stepIndex: number, step: any, currentStepIndex: number): boolean {
  ...
  // Không cho click vào các step đã active (đã có time)
  if (step.time && step.time.trim() !== '') {
    return false // Step đã có time (đã active), không cho click
  }
  
  // Tìm step tiếp theo (step đầu tiên chưa có time)
  let nextStepIndex = -1
  for (let i = 0; i < trackingSteps.value.length; i++) {
    const s = trackingSteps.value[i]
  ...
  // Nếu không tìm thấy step tiếp theo (tất cả đã active), không cho click
  if (nextStepIndex === -1) {
    return false
  }
  
  // Chỉ cho phép click vào step tiếp theo (step đầu tiên chưa có time)
  if (stepIndex !== nextStepIndex) {
    return false // Không phải step tiếp theo, không cho click
  }
  
  // Kiểm tra tất cả các bước trước đó đã có time chưa
  let allPreviousStepsHaveTime = true
  for (let i = 0; i < stepIndex; i++) {
    if (!trackingSteps.value[i].time || trackingSteps.value[i].time.trim() === '') {
  ...
  // 2. Là step tiếp theo (step đầu tiên chưa có time)
  // 3. Step chưa có time (chưa active)
  return allPreviousStepsHaveTime && stepIndex === nextStepIndex && !step.time
}
...
async function saveImeiToDatabase(imei: string, product?: any) {
  try {
    ...
    if (authStore.user?.id) {
      // Ưu tiên idNguoiDung nếu có (đây là ID thực sự của nhân viên)
      payload.nhanVienId = authStore.user.idNguoiDung || authStore.user.id
      console.log('👤 Thông tin nhân viên:', {
        userId: authStore.user.id,
        idNguoiDung: authStore.user.idNguoiDung,
        nhanVienId: payload.nhanVienId,
        hoTen: authStore.user.hoTen,
        username: authStore.user.username
      })
    } else {
      console.warn('⚠️ Không tìm thấy thông tin nhân viên trong authStore')
...
// Hủy IMEI (xóa IMEI khỏi hóa đơn và chuyển trạng thái về khả dụng)
async function cancelImei(imei: string) {
  console.log('🚀 CANCEL IMEI FUNCTION CALLED:', imei)

  if (!trackingData.value?.maHoaDon) {
    toastRef.value?.error('Lỗi', 'Không tìm thấy mã hóa đơn')
    return
  }

  try {
    console.log('Canceling IMEI:', imei)
    console.log('Mã hóa đơn:', trackingData.value.maHoaDon)
    ...
    console.log('🚀 Sending API request to:', `${API_BASE_URL}/api/hoa-don/remove-imei`)
    console.log('🚀 Request body:', requestBody)

    const response = await fetch(`${API_BASE_URL}/api/hoa-don/remove-imei`, {
      method: 'POST',
      headers: {
        'Content-Type': 'application/json',
      },
    ...
    // Đọc response body để lấy thông báo lỗi chi tiết
    const rawText = await response.text()
    if (!response.ok) {
      ...
      try {
        ...
        // Nếu không parse được JSON, dùng rawText
        if (rawText) {
    ...
    let errorMessage = 'Có lỗi xảy ra khi hủy IMEI'
    if (error.message) {
...
function showImeiDetailsForAllProducts() {
  if (trackingData.value && trackingData.value.danhSachSanPham && trackingData.value.danhSachSanPham.length > 0) {
    ...
    trackingData.value.danhSachSanPham.forEach((product, index) => {
      ...
      console.log(`🔍 Product ${index} imeis type:`, typeof product.imeis, Array.isArray(product.imeis))
      console.log(`🔍 Product ${index} imeis length:`, product.imeis?.length)
      console.log(`🔍 Product ${index} imeis content:`, JSON.stringify(product.imeis, null, 2))
      console.log(`🔍 Product ${index} imei (single):`, product.imei)
      console.log(`🔍 Product ${index} hasImei check:`, hasImei(product))
      console.log(`🔍 Product ${index} getAllImeis result:`, getAllImeis(product))
...
// Helper function để kiểm tra sản phẩm đã có IMEI chưa
function hasImei(product: any): boolean {
  // Kiểm tra product.imeis (array)
  if (product.imeis && Array.isArray(product.imeis) && product.imeis.length > 0) {
    // Kiểm tra xem có ít nhất 1 IMEI hợp lệ không
    const validImeis = product.imeis.filter((imei: any) => {
      const imeiValue = typeof imei === 'object' ? imei.imei : imei
      return imeiValue && String(imeiValue).trim() !== ''
    })
    return validImeis.length > 0
  }
...
// Helper function để lấy tất cả IMEI từ product (hỗ trợ cả array và single)
function getAllImeis(product: any): Array<{ imei: string, trangThai?: number }> {
  const imeis: Array<{ imei: string, trangThai?: number }> = []
  
  // Lấy từ product.imeis (array)
  if (product.imeis && Array.isArray(product.imeis) && product.imeis.length > 0) {
    product.imeis.forEach((imei: any) => {
      const imeiValue = typeof imei === 'object' ? imei.imei : imei
      if (imeiValue && String(imeiValue).trim() !== '') {
        imeis.push({
          imei: String(imeiValue).trim(),
          trangThai: typeof imei === 'object' ? imei.trangThai : 0
        })
      }
    })
  }
  
  // Lấy từ product.imei (single) nếu chưa có trong array
  if (product.imei && imeis.length === 0) {
    const imeiValue = typeof product.imei === 'object' ? product.imei.imei : product.imei
    if (imeiValue && String(imeiValue).trim() !== '') {
      imeis.push({
        imei: String(imeiValue).trim(),
        trangThai: typeof product.imei === 'object' ? product.imei.trangThai : 0
      })
    }
  }
...
async function addImeiToProduct(product: any) {
  ...
  // Kiểm tra sản phẩm đã có IMEI chưa - chỉ cho phép 1 IMEI mỗi sản phẩm
  if (hasImei(product)) {
    toastRef.value?.warning('Cảnh báo', 'Sản phẩm này đã có IMEI. Mỗi sản phẩm chỉ được phép có 1 IMEI.')
    return
  }
...
async function performAddImeiToProduct() {
  ...
  try {
    ...
    if (targetIndex !== -1 && trackingData.value?.danhSachSanPham[targetIndex]) {
      ...
      // Force reactivity cho modal - cập nhật cả array và single product
      if (Array.isArray(selectedProductForImei.value)) {
        // Nếu là array, cập nhật toàn bộ và đảm bảo product trong array cũng được cập nhật
        selectedProductForImei.value = trackingData.value.danhSachSanPham.map((p: any, idx: number) => {
          if (idx === targetIndex) {
            return { ...exactProduct, _index: idx }
          }
          return { ...p, _index: idx }
        })
        console.log('🔄 Forced reactivity for selectedProductForImei (array)')
        console.log('🔍 IMEIs in updated product:', exactProduct.imeis)
      } else if (selectedProductForImei.value) {
        // Nếu là single product, tìm và cập nhật product tương ứng
        const productIndex = selectedProductForImei.value._index !== undefined 
          ? selectedProductForImei.value._index 
          : trackingData.value.danhSachSanPham.findIndex((p: any) => 
              (p.chiTietHoaDonId && selectedProductForImei.value.chiTietHoaDonId && p.chiTietHoaDonId === selectedProductForImei.value.chiTietHoaDonId) ||
              (p.idChiTietHoaDon && selectedProductForImei.value.idChiTietHoaDon && p.idChiTietHoaDon === selectedProductForImei.value.idChiTietHoaDon)
            )
        
        if (productIndex === targetIndex && trackingData.value.danhSachSanPham[productIndex]) {
          // Cập nhật selectedProductForImei với dữ liệu mới nhất từ danhSachSanPham
          selectedProductForImei.value = { ...exactProduct, _index: productIndex }
          console.log('🔄 Forced reactivity for selectedProductForImei (single product)')
          console.log('🔍 IMEIs in updated product:', exactProduct.imeis)
          console.log('🔍 hasImei check:', hasImei(selectedProductForImei.value))
          console.log('🔍 getAllImeis result:', getAllImeis(selectedProductForImei.value))
        } else if (productIndex !== -1 && trackingData.value.danhSachSanPham[productIndex]) {
          // Nếu không phải cùng product, vẫn cập nhật để đảm bảo dữ liệu mới nhất
          selectedProductForImei.value = { ...trackingData.value.danhSachSanPham[productIndex], _index: productIndex }
        }
      }
    } else if (targetProduct) {
      // Fallback: nếu không tìm thấy trong danhSachSanPham, cập nhật trực tiếp vào product
      if (!targetProduct.imeis) {
        targetProduct.imeis = []
      }
      if (!targetProduct.imeis.some((x: any) => (typeof x === 'object' ? x.imei : x) === newImei)) {
        targetProduct.imeis.push({ imei: newImei, trangThai: 0 })
      }
      // Cập nhật selectedProductForImei nếu là single product
      if (selectedProductForImei.value && !Array.isArray(selectedProductForImei.value)) {
        selectedProductForImei.value = { ...targetProduct }
      }
    }
    ...
    // Refresh tracking data để lấy IMEI mới từ backend và đảm bảo hiển thị đúng
    try {
      await refreshTrackingData()
      console.log('✅ Refreshed tracking data after adding IMEI')
      
      // Cập nhật lại selectedProductForImei sau khi refresh
      if (targetIndex !== -1 && trackingData.value?.danhSachSanPham[targetIndex]) {
        const refreshedProduct = trackingData.value.danhSachSanPham[targetIndex]
        
        if (Array.isArray(selectedProductForImei.value)) {
          // Cập nhật product trong array
          selectedProductForImei.value = selectedProductForImei.value.map((p: any, idx: number) => {
            if (p._index === targetIndex || idx === targetIndex) {
              return { ...refreshedProduct, _index: targetIndex }
            }
            return p
          })
        } else if (selectedProductForImei.value) {
          // Cập nhật single product
          if (selectedProductForImei.value._index === targetIndex || 
              (selectedProductForImei.value.chiTietHoaDonId && refreshedProduct.chiTietHoaDonId && 
               selectedProductForImei.value.chiTietHoaDonId === refreshedProduct.chiTietHoaDonId)) {
            selectedProductForImei.value = { ...refreshedProduct, _index: targetIndex }
            console.log('✅ Updated selectedProductForImei after refresh:', selectedProductForImei.value)
          }
        }
      }
    } catch (refreshError) {
      console.error('⚠️ Error refreshing tracking data:', refreshError)
      // Vẫn hiển thị thành công vì đã cập nhật UI trước đó
    }
...
function getPaymentTypeLabel(loaiThanhToan: string | null | undefined): string {
  if (!loaiThanhToan) return 'Thanh toán'
  switch (loaiThanhToan.toUpperCase()) {
    case 'PAYMENT':
      return 'Thanh toán'
    case 'REFUND':
      return 'Hoàn phí'
...
function getPaymentTypeClass(loaiThanhToan: string | null | undefined): string {
  if (!loaiThanhToan) return 'payment-dot'
  switch (loaiThanhToan.toUpperCase()) {
    case 'PAYMENT':
      return 'payment-dot'
    case 'REFUND':
      return 'refund-dot'
    case 'ADDITIONAL_FEE':
      return 'additional-fee-dot'
    default:
      return 'payment-dot'
  }
}

function getPaymentAmountClass(loaiThanhToan: string | null | undefined): string {
  if (!loaiThanhToan) return ''
  switch (loaiThanhToan.toUpperCase()) {
    case 'PAYMENT':
    ...
    case 'ADDITIONAL_FEE':
      return 'additional-fee-amount'
    default:
      return ''
  }
}

function getPaymentTypeBadgeClass(loaiThanhToan: string | null | undefined): string {
  ...
  switch (loaiThanhToan.toUpperCase()) {
    ...
    case 'REFUND':
      return 'badge-refund'
    case 'ADDITIONAL_FEE':
      return 'badge-additional-fee'
    default:
      return 'badge-payment'
  }
}

// Helper functions for refund status display
function getRefundStatusLabel(trangThai: number | null | undefined): string {
  if (trangThai === null || trangThai === undefined) return 'Không xác định'
  switch (trangThai) {
...
function getRefundStatusBadgeClass(trangThai: number | null | undefined): string {
  if (trangThai === null || trangThai === undefined) return 'badge-refund-status-unknown'
  switch (trangThai) {
    case 0: // REFUND_PENDING
      return 'badge-refund-status-pending'
    case 1: // REFUND_TRANSFERRED
      return 'badge-refund-status-transferred'
    case 2: // REFUND_COMPLETED
      return 'badge-refund-status-completed'
    default:
      return 'badge-refund-status-unknown'
  }
}

// Handle transfer refund (Admin)
const isTransferringRefund = ref(false)
const showTransferRefundModal = ref(false)
const selectedRefundInfo = ref<{
...
// Helper function để lấy thông tin refund từ payment history
function getRefundInfo(refundId: number) {
  if (!trackingData.value?.lichSuThanhToan) return null
  
  const refund = trackingData.value.lichSuThanhToan.find(
    (payment: any) => payment.id === refundId && payment.loaiThanhToan === 'REFUND'
  )
  ...
  return {
    refundId: refund.id,
    soTien: refund.soTien,
    maGiaoDich: refund.maGiaoDich || 'Chưa có'
  }
}

async function handleTransferRefund(refundId: number) {
  if (!trackingData.value?.maHoaDon) return

  // Lấy thông tin refund
  const refundInfo = getRefundInfo(refundId)
  if (!refundInfo) {
  ...
  selectedRefundInfo.value = refundInfo
  showTransferRefundModal.value = true
}

async function confirmTransferRefund() {
  if (!selectedRefundInfo.value.refundId || !trackingData.value?.maHoaDon) return

  isTransferringRefund.value = true
  try {
    const { data } = await api.put(`/api/hoa-don/${trackingData.value.maHoaDon}/refund/${selectedRefundInfo.value.refundId}/transfer`)
    
    if (data && data.success) {
      toastRef.value?.success('Thành công', 'Đã chuyển tiền hoàn phí thành công')
      // Refresh tracking data to get updated refund status
      await refreshTrackingData()
      // Đóng modal
  ...
  <div class="hoa-don-detail-page">
    ...
    <div class="main-content">
      ...
      <div v-if="trackingData" class="main-content-container">
          ...
          <div class="info-card">
            ...
            <div class="info-item">
              ...
              <span class="delivery-method">
                    {{ trackingData.phuongThucNhanHang || 'Chưa xác định' }}
                  </span>
            </div>
            <div class="info-item">
        ...
        <!-- Row 2: History Combined Card + Order Summary -->
        <div class="row-container">
          <div class="info-card">
            <h3>
            ...
            <!-- Lịch Sử Hóa Đơn Section -->
            <div class="history-section">
              <h4 class="history-section-title">
            ...
            <div class="history-section">
              <h4 class="history-section-title">
                <font-awesome-icon icon="money-bill" class="mr-2" />
                Lịch Sử Thanh Toán
              </h4>
              <div v-if="trackingData && trackingData.lichSuThanhToan && Array.isArray(trackingData.lichSuThanhToan) && trackingData.lichSuThanhToan.length > 0" class="history-list">
                <div v-for="(payment, index) in trackingData.lichSuThanhToan" :key="`payment-${index}-${payment.ngayThanhToan || payment.id || index}`" class="history-item">
                  <div class="history-dot" :class="getPaymentTypeClass(payment.loaiThanhToan)"></div>
                  <div class="history-content">
                    <div class="history-action">
                      <span :class="getPaymentAmountClass(payment.loaiThanhToan)">
                        {{ payment.loaiThanhToan === 'REFUND' ? '-' : '' }}{{ formatCurrency(payment.soTien) }}
                      </span>
                      <span v-if="payment.phuongThucThanhToan" class="payment-method-badge">
                    ...
                    <div class="history-actor">
                      <span v-if="payment.maGiaoDich">Mã GD: {{ payment.maGiaoDich }}</span>
                      <span v-else-if="payment.trangThai === 0 || !payment.ngayThanhToan">Thanh toán khi nhận hàng</span>
                      <span v-else>{{ getPaymentTypeLabel(payment.loaiThanhToan) }}</span>
                    </div>
                  </div>
                  ...
                  <!-- Admin: Chuyển tiền button for refund (only show when status is PENDING) -->
                  <div v-if="payment.loaiThanhToan === 'REFUND' && payment.trangThai === 0" class="history-actions">
                    <button 
                      @click="handleTransferRefund(payment.id)" 
        ...
        <div class="flex-1 overflow-y-auto p-6">
          <div v-if="Array.isArray(selectedProductForImei)" class="space-y-6">
            <div v-for="(product, index) in selectedProductForImei" :key="product.id || index" class="space-y-4">
              ...
              <!-- IMEI Section -->
              <div class="space-y-3">
                <!-- Debug info (temporary - set to false to hide) -->
                <div v-if="false" class="text-xs text-gray-500 p-2 bg-gray-100 rounded mb-2">
                  <div><strong>Debug IMEI:</strong></div>
                  <div>hasImei: {{ hasImei(product) }}</div>
                  <div>getAllImeis length: {{ getAllImeis(product).length }}</div>
                  <div>getAllImeis: {{ JSON.stringify(getAllImeis(product)) }}</div>
                  <div>product.imeis type: {{ Array.isArray(product.imeis) ? 'Array' : typeof product.imeis }}</div>
                  <div>product.imeis length: {{ product.imeis?.length || 0 }}</div>
                  <div>product.imeis: {{ JSON.stringify(product.imeis) }}</div>
                  <div>product.imei: {{ JSON.stringify(product.imei) }}</div>
                </div>
                
                <!-- Input và buttons chỉ hiển thị khi chưa có IMEI -->
                <div v-if="!hasImei(product) && trackingData?.trangThai !== 3 && trackingData?.trangThai !== 4" class="flex items-center gap-3">
                ...
                <!-- Existing IMEI List -->
                <div v-if="hasImei(product)" class="space-y-3">
                  <h5 class="font-semibold text-gray-700">IMEI đã thêm:</h5>
                  <div v-for="(imeiObj, imeiIndex) in getAllImeis(product)" :key="`imei-${imeiIndex}-${imeiObj.imei}`"
                    ...
                    <div class="flex-1">
                      <div class="text-sm text-gray-600 mb-1">Mã IMEI:</div>
                      <div class="font-bold text-gray-800 text-lg font-mono">{{ imeiObj.imei }}</div>
                    </div>
                    <div class="flex items-center gap-2">
                      ...
                      <button
                        ...
                        @click="cancelImei(imeiObj.imei)"
                        class="px-3 py-1.5 bg-red-500 hover:bg-red-600 rounded-lg flex items-center gap-2 text-white text-sm transition-colors"
                        title="Gỡ IMEI khỏi sản phẩm"
                      >
          ...
          <div v-else-if="selectedProductForImei" class="space-y-4">
            ...
            <div class="space-y-3">
              <!-- Input và buttons chỉ hiển thị khi chưa có IMEI -->
              <div v-if="!hasImei(selectedProductForImei) && trackingData?.trangThai !== 3 && trackingData?.trangThai !== 4" class="flex items-center gap-3">
                <h5 class="font-semibold text-gray-700">Mã IMEI:</h5>
                <input
              ...
              <!-- Existing IMEI List -->
              <div v-if="hasImei(selectedProductForImei)" class="space-y-3">
                <h5 class="font-semibold text-gray-700">IMEI đã thêm:</h5>
                <div v-for="(imeiObj, imeiIndex) in getAllImeis(selectedProductForImei)" :key="`imei-${imeiIndex}-${imeiObj.imei}`"
                     class="bg-gradient-to-r from-orange-50 to-yellow-50 border-2 border-orange-300 rounded-lg p-4 flex items-center justify-between">
                  <div class="flex-1">
                    <div class="text-sm text-gray-600 mb-1">Mã IMEI:</div>
                    <div class="font-bold text-gray-800 text-lg font-mono">{{ imeiObj.imei }}</div>
          ...
          <div class="transfer-confirm-content">
            ...
            <div class="transfer-info-box">
              <div class="transfer-info-item">
                ...
                <span class="transfer-value">{{ trackingData?.maHoaDon || 'N/A' }}</span>
              </div>
              <div class="transfer-info-item">
...
/* History Actions (for refund buttons) */
.history-actions {
  display: flex;
  align-items: center;
  gap: 8px;
