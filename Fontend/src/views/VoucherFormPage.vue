      ...
      <form @submit.prevent="handleSubmit" class="voucher-form">
        ...
        <div class="form-section">
          ...
          <div class="form-row" v-if="formData.loaiPhieuGiamGia === 'FIXED'">
            ...
            <div class="form-group">
              <label for="soLuongDung">
                Số Lượng Sử Dụng:
                <span v-if="!formData.riengTu" class="required">*</span>
                <span v-else class="help-text">(Tự động tính theo số khách hàng được chọn)</span>
              </label>
              <input
                id="soLuongDung"
                type="number"
...
function validateForm(): boolean {
  ...
  // Validate số lượng sử dụng (chỉ validate nếu không phải voucher riêng tư)
...
async function confirmSubmit() {
  ...
  try {
    // Nếu là voucher riêng tư, số lượng sử dụng = số khách hàng được chọn
    const soLuongDung = formData.value.riengTu
      ? selectedCustomers.value.length
      : formData.value.soLuongDung
...
async function handlePrivateVoucherChange() {
  if (formData.value.riengTu) {
    // Xóa số lượng sử dụng khi chọn voucher riêng tư (số lượng sẽ tính theo số khách hàng được chọn)
    formData.value.soLuongDung = 0
    ...
    // Khôi phục số lượng mặc định khi bỏ chọn voucher riêng tư
    if (formData.value.soLuongDung === 0) {
