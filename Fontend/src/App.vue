<template>
  ...
  <div v-else id="app" class="h-screen bg-gradient-to-br from-slate-50 to-blue-50">
    <div class="flex h-full">
      ...
      >
        ...
        <div class="sidebar-header">
          <div class="flex items-center justify-center w-full relative">
            <!-- Logo -->
            <img v-if="!sidebarCollapsed" src="/logo.png" alt="Logo" class="logo-image" />

            <!-- Toggle Button -->
            <button class="toggle-btn absolute right-0" @click="toggleSidebar">
        ...
        <nav class="navigation-menu">
            ...
            >
              ...
              <div v-if="!sidebarCollapsed" class="nav-arrow">
                <svg
                  ...
                  :class="{ 'rotate-180': showThuocTinh }"
                  fill="none"
                  stroke="currentColor"
                  viewBox="0 0 24 24"
                >
          ...
          >
            ...
            <span v-if="!sidebarCollapsed" class="nav-text">{{
              t('navigation.discountCampaigns')
            }}</span>
          </router-link>

          <!-- Divider -->
          <div
            v-if="!sidebarCollapsed && (authStore.isManager || authStore.isAdmin)"
            class="nav-divider"
          >
      ...
      <!-- Main Content -->
      <main class="flex-1 overflow-y-auto h-full" :class="{ 'full-width': isLoginPage }">
        <router-view />
      </main>
    </div>
...
let productStatusStompClient = null
...
onMounted(async () => {
  ...
  // Initialize WebSocket for product status updates (for website cart)
  try {
    const socket = new SockJS('/ws')
    productStatusStompClient = Stomp.over(socket)
    productStatusStompClient.debug = null // Disable debug logs

    productStatusStompClient.connect(
      {},
      () => {
        console.log('WebSocket connected for product status updates (website)')
        // Subscribe to product status updates
        productStatusStompClient.subscribe('/topic/product-status', (message) => {
          try {
            const data = JSON.parse(message.body)
            console.log('Received product status update (website):', data)
            if (data.type === 'PRODUCT_DEACTIVATED' || data.type === 'PRODUCT_DETAIL_DEACTIVATED') {
              const chiTietSanPhamId = data.chiTietSanPhamId
              if (chiTietSanPhamId) {
                // Remove product from cart
                cartStore.removeItem(chiTietSanPhamId)
                // Show notification (you can add a toast notification here if needed)
                console.log(
                  `Sản phẩm "${data.tenSanPham || 'Sản phẩm'}" đã ngừng hoạt động và đã được xóa khỏi giỏ hàng`,
                )
              }
            }
          } catch (error) {
      ...
      (error) => {
        console.error('WebSocket error (website):', error)
      },
    )
  } catch (error) {
    console.error('Failed to initialize WebSocket for product status (website):', error)
  }
})

onUnmounted(() => {
  // Disconnect WebSocket
  if (productStatusStompClient && productStatusStompClient.connected) {
    productStatusStompClient.disconnect()
    productStatusStompClient = null
  }
})
