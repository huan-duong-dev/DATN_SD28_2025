...
class ShippingService {
  ...
  async calculateShippingFee(provinceId, districtId, weight = 1000) {
    ...
    try {
      ...
      // Always rely on GHN fee calculation
      const ghnFee = await this.calculateGHNFee(districtId, weight)
      const expressFee = this.deriveExpressFeeFromGHN(ghnFee)
  ...
  // Derive express fee from GHN fee to keep ordering consistent
  deriveExpressFeeFromGHN(ghnFee) {
    // If GHN fee fails, fall back to sensible default
    if (!ghnFee || ghnFee <= 0) {
      return 30000
    }

    // Discount GHN price to make express cheaper but still based on real fee
    const discounted = Math.round(ghnFee * 0.8)
    const minExpress = 20000
    let expressFee = Math.max(discounted, minExpress)

    // Ensure express is always strictly cheaper
    if (expressFee >= ghnFee) {
      expressFee = Math.max(ghnFee - 10000, minExpress)
    }
    ...
    return {
      standard: {
        ...
        ghn: {
          name: 'Ship hỏa tốc',
          description: 'Giao hàng hỏa tốc',
          price: 45000, // Ensure GHN is more expensive than express
          estimatedDays: '1-2 ngày',
          provider: 'GHN'
        }
    }
