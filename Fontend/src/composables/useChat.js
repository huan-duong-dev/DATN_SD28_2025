...
export function useChat() {
    ...
    try {
      ...
      if (saved) {
        ...
        if (timeDiff < 3600000) {
          ...
          // Find selectedChat from activeChats or waitingCustomers to maintain reference
          if (data.selectedChat && data.selectedChat.sessionId) {
            // Try to find in activeChats first
            let foundChat = activeChats.value.find(c => c.sessionId === data.selectedChat.sessionId)
            // If not found, try waitingCustomers
            if (!foundChat) {
              foundChat = waitingCustomers.value.find(c => c.sessionId === data.selectedChat.sessionId)
            }
            
            if (foundChat) {
              // Use the reference from activeChats/waitingCustomers
              selectedChat.value = foundChat
              // Load messages from the chat object (which should have messages from saved data)
              if (foundChat.messages && Array.isArray(foundChat.messages) && foundChat.messages.length > 0) {
                messages.value = [...foundChat.messages]
                console.log('✅ Loaded messages from chat object:', foundChat.messages.length, 'messages')
              } else {
                // Fallback: use messages from saved selectedChat if chat object doesn't have them
                messages.value = Array.isArray(data.messages) ? data.messages : []
                // Also restore messages to chat object
                if (messages.value.length > 0) {
                  foundChat.messages = [...messages.value]
                  console.log('✅ Restored messages to chat object:', messages.value.length, 'messages')
                } else {
                  messages.value = []
                  foundChat.messages = []
                }
              ...
              // Chat not found in activeChats or waitingCustomers, use saved data
              selectedChat.value = data.selectedChat
              messages.value = Array.isArray(data.messages) ? data.messages : []
              console.log('⚠️ Selected chat not found in active/waiting lists, using saved data')
            }
          } else {
          ...
          console.log('📂 Staff session data loaded successfully:', {
            ...
            activeCount: activeChats.value.length,
            selectedChat: selectedChat.value?.customerName || 'none',
            selectedChatSessionId: selectedChat.value?.sessionId || 'none',
            messageCount: messages.value.length,
            chatMessagesCount: selectedChat.value?.messages?.length || 0
          })
          return true
        } else {
    ...
    try {
      ...
      import('sockjs-client').then(sockjsModule => {
        import('stompjs').then(stompModule => {
          ...
          // Use relative path to work with Vite proxy on both ports (5173 and 5174)
          const socketUrl = '/ws'
  ...
  const selectChat = (chat) => {
    ...
    if (chat.messages && Array.isArray(chat.messages) && chat.messages.length > 0) {
      ...
      console.log('📨 Messages content:', chat.messages)
      messages.value = [...chat.messages] // Use spread to avoid reference issues
      console.log('✅ Messages loaded to display:', messages.value.length)
    } else {
      console.log('📨 No existing messages in chat object, initializing empty array')
      console.log('📨 Chat.messages value:', chat.messages)
      messages.value = []
    ...
    saveStaffSessionData()
    console.log('✅ Chat selected successfully:', chat.sessionId, 'Messages displayed:', messages.value.length)
  }
        ...
        break
             case 'customer_assigned':
               ...
               if (!existingChat) {
                 ...
                 // Try to find customer in waiting list to preserve messages
                 const waitingCustomer = waitingCustomers.value.find(c => c.sessionId === data.sessionId)
                 
                 // Add to active chats, preserving messages if customer was in waiting list
                 activeChats.value.push({
                   sessionId: data.sessionId,
                   customerName: data.customerName,
                   phone: data.phone,
                   email: data.email,
                   joinedAt: waitingCustomer?.joinedAt || new Date(),
                   messages: waitingCustomer?.messages ? [...waitingCustomer.messages] : [] // Preserve existing messages
                 })
                 
                 console.log('✅ Customer moved to active chats, messages preserved:', waitingCustomer?.messages?.length || 0)
               } else {
  ...
  const handleCustomerJoined = (data) => {
    ...
    if (data.success) {
      ...
      // Send initial message if provided in form
      if (customerForm.value.message && customerForm.value.message.trim()) {
        console.log('📤 Sending initial message from form:', customerForm.value.message)
        ...
        // Also add to customer chat data
        if (customerChatData.value) {
          customerChatData.value.messages.push(initialMessage)
        }
        ...
        if (socket.value && sessionId.value) {
          try {
            socket.value.send('/app/customer/send-message', {}, JSON.stringify({
              sessionId: sessionId.value,
              message: customerForm.value.message
            }))
  ...
  const handleStaffMessage = (data) => {
    ...
    switch (data.type) {
      ...
      case 'customer_message':
        ...
        // Find the chat in activeChats or waitingCustomers
        let targetChat = activeChats.value.find(c => c.sessionId === data.sessionId)
        if (!targetChat) {
          targetChat = waitingCustomers.value.find(c => c.sessionId === data.sessionId)
        }
        ...
        // Always save message to chat object (whether selected or not)
        if (targetChat) {
          ...
          // Check for duplicate in chat's messages
          const messageExistsInChat = targetChat.messages.some(msg => {
            ...
            return contentMatch && senderMatch && timeDiff < 3000
          })
          
          if (!messageExistsInChat) {
            targetChat.messages.push(newMessage)
            console.log('✅ Added customer message to chat object:', data.message, 'sessionId:', data.sessionId)
          } else {
            console.log('⚠️ Duplicate message in chat object, skipping:', data.message)
          }
        } else {
          console.log('⚠️ Chat not found for sessionId:', data.sessionId)
        }
        
        // If this chat is currently selected, also add to messages.value for display
        if (selectedChat.value && selectedChat.value.sessionId === data.sessionId) {
          ...
          if (!messageExists) {
            messages.value.push(newMessage)
            console.log('✅ Added customer message to selected chat display:', data.message)
          } else {
            console.log('⚠️ Duplicate message in selected chat, skipping:', data.message)
          }
        }
  ...
  onMounted(() => {
    ...
    // Always init WebSocket - needed for both staff (with token) and customers (without token)
    // Staff will auto-join when connected, customers will join when they start chat
    ...
    watch(() => authStore.token, (newToken) => {
      if (newToken && isConnected.value && !isStaff.value) {
        // If token becomes available and we're connected but not staff yet, join as staff
        console.log('Auth token available, joining as staff')
        joinAsStaff()
      } else if (!newToken && isStaff.value) {
        // If token is removed and we're staff, leave as staff but keep WebSocket for customer use
        console.log('Auth token removed, leaving as staff')
        leaveAsStaff()
      }
    })
