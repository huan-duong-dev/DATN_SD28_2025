...
public class ChatService {
    ...
    public void staffTakeCustomer(String staffId, String sessionId) {
        ...
        if (session != null && onlineStaff.containsKey(staffId)) {
            ...
            // Send all saved messages to staff (messages sent before staff assignment)
            if (session.getMessages() != null && !session.getMessages().isEmpty()) {
                System.out.println("📨 Sending " + session.getMessages().size() + " saved messages to staff");
                for (ChatMessage savedMessage : session.getMessages()) {
                    if ("customer".equals(savedMessage.getSender())) {
                        // Send each saved customer message to staff
                        messagingTemplate.convertAndSend("/topic/staff/" + staffId, Map.of(
                        ...
                        System.out.println("✅ Sent saved message: " + savedMessage.getContent());
                    }
                }
            } else {
                System.out.println("📭 No saved messages to send");
            }
    ...
    public void sendCustomerMessage(String sessionId, String message) {
        ...
        if (session != null) {
            ...
            // Always save message to session, even if no staff assigned yet
            ...
            System.out.println("✅ Message saved to session. Total messages: " + session.getMessages().size());
            ...
            if (session.getAssignedStaffId() != null) {
                ...
                System.out.println("⚠️ No staff assigned yet, message saved for later delivery");
            }
        } else {
    ...
    public static class CustomerSession {
        ...
        private List<ChatMessage> messages; // Store messages even before staff assignment
