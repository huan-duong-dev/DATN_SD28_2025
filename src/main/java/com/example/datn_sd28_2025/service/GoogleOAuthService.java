...
public class GoogleOAuthService {
    ...
    public Map<String, Object> authenticateGoogleUser(String code) {
        return authenticateGoogleUser(code, null);
    }

    /**
     * Xác thực Google token và lấy thông tin người dùng (với redirect URI tùy chỉnh)
     */
    public Map<String, Object> authenticateGoogleUser(String code, String redirectUri) {
        try {
    ...
    /**
     * Lấy access token từ Google (với redirect URI tùy chỉnh)
     */
    private String getAccessToken(String code, String customRedirectUri) {
        try {
            ...
            // Sử dụng customRedirectUri nếu có, nếu không thì dùng từ config
            String redirectUri = (customRedirectUri != null && !customRedirectUri.trim().isEmpty()) 
                    ? customRedirectUri 
                    : googleOAuthConfig.getRedirectUri();

            // URL decode code nếu cần (code có thể đã được encode trong URL)
            try {
                code = java.net.URLDecoder.decode(code, "UTF-8");
            } catch (Exception e) {
                // Nếu decode fail, sử dụng code gốc
                System.out.println("GOOGLE OAUTH - Code decode warning: " + e.getMessage());
            }

            System.out.println("GOOGLE OAUTH - CODE (after decode): " + code);
            System.out.println("GOOGLE OAUTH - CLIENT_ID: " + clientId);
            System.out.println("GOOGLE OAUTH - CLIENT_SECRET: " + (clientSecret != null ? "***" : "NULL"));
            ...
            // Sử dụng LinkedMultiValueMap để đảm bảo thứ tự và encoding đúng
            MultiValueMap<String, String> body = new LinkedMultiValueMap<>();
            body.add("code", code);
            body.add("client_id", clientId);
            body.add("client_secret", clientSecret);
            body.add("redirect_uri", redirectUri); // Spring sẽ tự động URL encode khi gửi
            ...
            System.out.println("GOOGLE OAUTH - TOKEN RESPONSE STATUS: " + response.getStatusCode());
            Map<String, Object> responseBody = response.getBody();
            if (responseBody != null) {
                System.out.println("GOOGLE OAUTH - TOKEN RESPONSE BODY: " + responseBody);
            ...
            System.out.println("GOOGLE OAUTH - ERROR STATUS: " + ex.getStatusCode());
            System.out.println("GOOGLE OAUTH - ERROR MESSAGE: " + ex.getMessage());
            ...
            if (ex.getStatusCode().value() == 400) {
                System.out.println("GOOGLE OAUTH - 400 Bad Request - Có thể do:");
                System.out.println("  1. Authorization code đã hết hạn (thường là 1 phút)");
                System.out.println("  2. Authorization code đã được sử dụng (chỉ dùng được 1 lần)");
                System.out.println("  3. Redirect URI không khớp với cấu hình trong Google Console");
                System.out.println("  4. Client ID hoặc Client Secret không đúng");
            }
    ...
    private KhachHang findOrCreateCustomer(Map<String, Object> userInfo) {
        ...
        List<KhachHang> customersByEmail = khachHangRepository.findByEmail(email);
        if (customersByEmail != null && !customersByEmail.isEmpty()) {
            KhachHang customerByEmail = customersByEmail.get(0); // Lấy khách hàng đầu tiên
            ...
            customerByEmail.setNguoiCapNhat("GOOGLE_OAUTH");
    ...
    public String getRedirectUri() {
        return googleOAuthConfig.getRedirectUri();
    }
