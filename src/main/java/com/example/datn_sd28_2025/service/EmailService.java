...
public class EmailService {
    ...
    /**
     * Gửi email biên bản tiếp nhận bảo hành với PDF đính kèm
     */
    public void sendBienBanTiepNhanBaoHanh(String toEmail, String customerName, String maPhieu, byte[] pdfBytes) {
        try {
            MimeMessage mime = mailSender.createMimeMessage();
            MimeMessageHelper helper = new MimeMessageHelper(mime, true, "UTF-8");
            helper.setFrom(fromEmail);
            helper.setTo(toEmail);
            helper.setSubject(String.format("%s - Biên bản tiếp nhận bảo hành %s", storeName, maPhieu));

            String emailBody = String.format("""
                <html>
                <body style="font-family: Arial, sans-serif; line-height: 1.6; color: #333;">
                    <div style="max-width: 600px; margin: 0 auto; padding: 20px;">
                        <h2 style="color: %s;">Xin chào %s,</h2>
                        
                        <p>Cảm ơn bạn đã sử dụng dịch vụ bảo hành tại <strong>%s</strong>.</p>
                        
                        <p>Chúng tôi đã tiếp nhận yêu cầu bảo hành của bạn với thông tin sau:</p>
                        
                        <div style="background-color: #f5f5f5; padding: 15px; border-radius: 5px; margin: 20px 0;">
                            <p><strong>Mã phiếu bảo hành:</strong> %s</p>
                            <p><strong>Ngày tiếp nhận:</strong> %s</p>
                        </div>
                        
                        <p>Biên bản tiếp nhận bảo hành chi tiết được đính kèm trong file PDF. Vui lòng lưu giữ file này để theo dõi quá trình xử lý bảo hành.</p>
                        
                        <p>Chúng tôi sẽ liên hệ với bạn khi có cập nhật về tình trạng bảo hành.</p>
                        
                        <p>Nếu bạn có bất kỳ câu hỏi nào, vui lòng liên hệ với chúng tôi:</p>
                        <ul>
                            <li>Hotline: 0919965342</li>
                            <li>Email: support@phonix.local</li>
                        </ul>
                ...
                """, primaryColor, safe(customerName), storeName, safe(maPhieu), 
            ...
            if (pdfBytes != null && pdfBytes.length > 0) {
                ...
                helper.addAttachment("BienBanTiepNhanBaoHanh_" + safe(maPhieu) + ".pdf", 
                    pdfResource, "application/pdf");
            }

            mailSender.send(mime);
            System.out.println("Biên bản tiếp nhận bảo hành email sent to: " + toEmail);

        } catch (Exception e) {
            System.err.println("Error sending biên bản tiếp nhận bảo hành email: " + e.getMessage());
            e.printStackTrace();
            // Không throw exception để không làm gián đoạn quá trình tiếp nhận
        }
    }
}
