...
public interface HoaDonRepository extends JpaRepository<HoaDon, Integer> {
    ...
    @Query(value = "SELECT h.* FROM hoa_don h " +
            "WHERE h.id IN (" +
            "  SELECT DISTINCT h1.id FROM hoa_don h1 " +
            "  WHERE h1.ma_hoa_don LIKE CONCAT('%', :keyword, '%') " +
            "  OR (h1.id_khach_hang IS NOT NULL AND EXISTS (SELECT 1 FROM khach_hang k WHERE k.id = h1.id_khach_hang AND k.ho_ten LIKE CONCAT('%', :keyword, '%'))) " +
            "  OR (h1.ten_khach_hang IS NOT NULL AND h1.ten_khach_hang LIKE CONCAT('%', :keyword, '%')) " +
            "  OR (h1.so_dien_thoai IS NOT NULL AND h1.so_dien_thoai LIKE CONCAT('%', :keyword, '%')) " +
            "  OR h1.id IN (SELECT DISTINCT hdct.id_hoa_don FROM hoa_don_chi_tiet hdct " +
            "    INNER JOIN imei_da_ban idb ON idb.id_hoa_don_chi_tiet = hdct.id " +
            "    WHERE LTRIM(RTRIM(idb.imei)) = LTRIM(RTRIM(:keyword)))" +
            ")",
            countQuery = "SELECT COUNT(DISTINCT h.id) FROM hoa_don h " +
            "WHERE h.ma_hoa_don LIKE CONCAT('%', :keyword, '%') " +
            "OR (h.id_khach_hang IS NOT NULL AND EXISTS (SELECT 1 FROM khach_hang k WHERE k.id = h.id_khach_hang AND k.ho_ten LIKE CONCAT('%', :keyword, '%'))) " +
            "OR (h.ten_khach_hang IS NOT NULL AND h.ten_khach_hang LIKE CONCAT('%', :keyword, '%')) " +
            "OR (h.so_dien_thoai IS NOT NULL AND h.so_dien_thoai LIKE CONCAT('%', :keyword, '%')) " +
            "OR h.id IN (SELECT DISTINCT hdct.id_hoa_don FROM hoa_don_chi_tiet hdct " +
            "  INNER JOIN imei_da_ban idb ON idb.id_hoa_don_chi_tiet = hdct.id " +
            "  WHERE LTRIM(RTRIM(idb.imei)) = LTRIM(RTRIM(:keyword)))",
            nativeQuery = true)
    ...
    // Update nhanVienId directly using native query
    @Modifying
    @Query(value = "UPDATE hoa_don SET id_nhan_vien = :nhanVienId WHERE id = :hoaDonId", nativeQuery = true)
    int updateNhanVienId(@Param("hoaDonId") Integer hoaDonId, @Param("nhanVienId") Integer nhanVienId);
