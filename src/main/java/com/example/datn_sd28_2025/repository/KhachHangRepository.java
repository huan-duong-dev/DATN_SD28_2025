...
public interface KhachHangRepository extends JpaRepository<KhachHang, Integer> {
    ...
    @Query("SELECT k FROM KhachHang k WHERE LOWER(k.email) = LOWER(:email) AND k.trangThai = 1 ORDER BY k.id DESC")
    ...
    @Query("SELECT k FROM KhachHang k WHERE k.taiKhoan = :taiKhoan AND k.trangThai = 1 ORDER BY k.id DESC")
    List<KhachHang> findByTaiKhoan(@Param("taiKhoan") String taiKhoan);
    
    @Query("SELECT k FROM KhachHang k WHERE (LOWER(k.email) = LOWER(:identifier) OR k.taiKhoan = :identifier) AND k.trangThai = 1 ORDER BY k.id DESC")
    List<KhachHang> findByEmailOrTaiKhoan(@Param("identifier") String identifier);
