...
public interface ChiTietHoaDonRepository extends JpaRepository<ChiTietHoaDon, Integer> {
    ...
    // Kiểm tra khách hàng đã mua sản phẩm chưa (đơn hàng đã hoàn thành - trangThai = 3)
    // Sử dụng native query vì ChiTietHoaDon có sanPhamId là Integer, không có quan hệ với ChiTietSanPham
    @Query(value = "SELECT COUNT(*) > 0 FROM chi_tiet_hoa_don c " +
           "JOIN hoa_don h ON h.id = c.hoa_don_id " +
           "WHERE h.id_khach_hang = :khachHangId " +
           "AND c.san_pham_id = :sanPhamId " +
           "AND h.trang_thai = 3", nativeQuery = true)
    boolean hasCustomerPurchasedProduct(@Param("khachHangId") Integer khachHangId, 
