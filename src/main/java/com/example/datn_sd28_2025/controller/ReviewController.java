...
public class ReviewController {
    ...
    // API endpoints cho chi tiết sản phẩm
    @GetMapping("/chi-tiet-san-pham/{idChiTietSanPham}")
    public ResponseEntity<List<ReviewDTO>> getReviewsByChiTietSanPham(@PathVariable Integer idChiTietSanPham) {
        try {
            List<ReviewDTO> reviews = reviewService.getReviewsByChiTietSanPhamId(idChiTietSanPham);
            return ResponseEntity.ok(reviews);
        } catch (Exception e) {
            return ResponseEntity.ok(List.of()); // Return empty list on error
        }
    ...
    @GetMapping("/chi-tiet-san-pham/{idChiTietSanPham}/rating")
    public ResponseEntity<Map<String, Object>> getRatingInfoByChiTietSanPham(@PathVariable Integer idChiTietSanPham) {
        try {
            Double averageRating = reviewService.getAverageRatingByChiTietSanPhamId(idChiTietSanPham);
            Long reviewCount = reviewService.getReviewCountByChiTietSanPhamId(idChiTietSanPham);
    ...
    // API kiểm tra khách hàng đã mua sản phẩm chưa
    @GetMapping("/check-purchase/{khachHangId}/{sanPhamId}")
    public ResponseEntity<Map<String, Object>> checkCustomerPurchase(
            @PathVariable Integer khachHangId,
            @PathVariable Integer sanPhamId) {
        try {
            boolean hasPurchased = reviewService.hasCustomerPurchasedProduct(khachHangId, sanPhamId);
            Map<String, Object> response = new HashMap<>();
            response.put("hasPurchased", hasPurchased);
            return ResponseEntity.ok(response);
        } catch (Exception e) {
            Map<String, Object> error = new HashMap<>();
            error.put("hasPurchased", false);
            error.put("error", "Có lỗi xảy ra khi kiểm tra");
            return ResponseEntity.ok(error);
        }
    }
}
