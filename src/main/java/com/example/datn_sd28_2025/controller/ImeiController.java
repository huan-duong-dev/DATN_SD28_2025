...
public class ImeiController {
    ...
    public ResponseEntity<List<ImeiDTO>> getByChiTietSanPham(@PathVariable Integer chiTietSanPhamId) {
        // Service method already filters to only active IMEIs (trangThai = 1)
        List<ImeiDTO> imeis = imeiService.getByChiTietSanPham(chiTietSanPhamId);
        return ResponseEntity.ok(imeis);
    }

    @GetMapping("/search")
    public ResponseEntity<?> searchByImei(@RequestParam String imei) {
        try {
            return ResponseEntity.ok(imeiService.searchByImei(imei));
        } catch (RuntimeException e) {
            ...
            return ResponseEntity.status(HttpStatus.INTERNAL_SERVER_ERROR).body(
                java.util.Map.of("error", "Lỗi khi tìm kiếm IMEI: " + e.getMessage())
            );
        }
    }
}
