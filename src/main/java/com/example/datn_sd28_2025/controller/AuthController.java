...
public class AuthController {
    ...
    /**
     * Lấy danh sách giao ca đang chờ xác nhận cho nhân viên
     */
    ...
    public ResponseEntity<?> getPendingShiftHandover(@RequestParam Integer nhanVienId) {
        try {
            if (nhanVienId == null) {
                return ResponseEntity.badRequest()
                    .body(Map.of("success", false, "message", "nhanVienId không được để trống"));
            }

            // Lấy danh sách giao ca chờ xác nhận cho nhân viên này
            java.util.List<GiaoCaDTO> pendingHandovers = giaoCaService.getPendingGiaoCaForNhanVienId(nhanVienId);
            
            return ResponseEntity.ok(pendingHandovers);
        } catch (Exception e) {
            e.printStackTrace();
            return ResponseEntity.internalServerError()
                .body(Map.of("success", false, "message", "Lỗi hệ thống: " + e.getMessage()));
    ...
    public ResponseEntity<?> confirmShiftHandover(@RequestBody Map<String, Object> request) {
        try {
            Integer giaoCaId = null;
            Integer nhanVienNhanId = null;
            
            // Lấy giaoCaId từ request
            if (request.get("giaoCaId") != null) {
                if (request.get("giaoCaId") instanceof Integer) {
                    giaoCaId = (Integer) request.get("giaoCaId");
                } else if (request.get("giaoCaId") instanceof Number) {
                    giaoCaId = ((Number) request.get("giaoCaId")).intValue();
                } else {
                    giaoCaId = Integer.parseInt(request.get("giaoCaId").toString());
                }
            }
            
            // Lấy nhanVienNhanId từ request
            if (request.get("nhanVienNhanId") != null) {
                if (request.get("nhanVienNhanId") instanceof Integer) {
                    nhanVienNhanId = (Integer) request.get("nhanVienNhanId");
                } else if (request.get("nhanVienNhanId") instanceof Number) {
                    nhanVienNhanId = ((Number) request.get("nhanVienNhanId")).intValue();
                } else {
                    nhanVienNhanId = Integer.parseInt(request.get("nhanVienNhanId").toString());
                }
            }

            if (giaoCaId == null) {
                return ResponseEntity.badRequest()
                    .body(Map.of("success", false, "message", "giaoCaId không được để trống"));
            }

            if (nhanVienNhanId == null) {
                return ResponseEntity.badRequest()
                    .body(Map.of("success", false, "message", "nhanVienNhanId không được để trống"));
            }

            // Tạo GiaoCaDTO từ request (nếu có các field khác)
            GiaoCaDTO giaoCaDTO = new GiaoCaDTO();
            if (request.get("soTienCuoiCa") != null) {
                giaoCaDTO.setSoTienCuoiCa(new java.math.BigDecimal(request.get("soTienCuoiCa").toString()));
            }
            if (request.get("soTienThuThem") != null) {
                giaoCaDTO.setSoTienThuThem(new java.math.BigDecimal(request.get("soTienThuThem").toString()));
            }
            if (request.get("soTienChiRa") != null) {
                giaoCaDTO.setSoTienChiRa(new java.math.BigDecimal(request.get("soTienChiRa").toString()));
            }
            if (request.get("tongDoanhThu") != null) {
                giaoCaDTO.setTongDoanhThu(new java.math.BigDecimal(request.get("tongDoanhThu").toString()));
            }
            if (request.get("soDonHang") != null) {
                giaoCaDTO.setSoDonHang(Integer.parseInt(request.get("soDonHang").toString()));
            }
            if (request.get("soDonHangThanhToanTienMat") != null) {
                giaoCaDTO.setSoDonHangThanhToanTienMat(Integer.parseInt(request.get("soDonHangThanhToanTienMat").toString()));
            }
            if (request.get("soDonHangThanhToanChuyenKhoan") != null) {
                giaoCaDTO.setSoDonHangThanhToanChuyenKhoan(Integer.parseInt(request.get("soDonHangThanhToanChuyenKhoan").toString()));
            }
            if (request.get("baoCaoCongViec") != null) {
                giaoCaDTO.setBaoCaoCongViec(request.get("baoCaoCongViec").toString());
            }
            if (request.get("suCoBatThuong") != null) {
                giaoCaDTO.setSuCoBatThuong(request.get("suCoBatThuong").toString());
            }
            if (request.get("congViecTonDong") != null) {
                giaoCaDTO.setCongViecTonDong(request.get("congViecTonDong").toString());
            }
            if (request.get("ghiChu") != null) {
                giaoCaDTO.setGhiChu(request.get("ghiChu").toString());
            }
            ...
            Map<String, Object> response = new HashMap<>();
            response.put("success", true);
            response.put("message", "Xác nhận giao ca thành công");
            response.put("giaoCa", confirmedGiaoCa);
