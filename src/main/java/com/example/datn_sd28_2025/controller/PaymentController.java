...
import com.example.datn_sd28_2025.controller.HoaDonController;
...
public class PaymentController {
    ...
    public PaymentController(VNPayService vnPayService, ZaloPayService zaloPayService, ZaloPayMockService zaloPayMockService, 
                            HoaDonService hoaDonService, HoaDonServiceImpl hoaDonServiceImpl,
                            HoaDonController hoaDonController, VNPayConfig config) {
        this.vnPayService = vnPayService;
        this.zaloPayService = zaloPayService;
        this.zaloPayMockService = zaloPayMockService;
        this.hoaDonService = hoaDonService;
        this.hoaDonServiceImpl = hoaDonServiceImpl;
    ...
    public ResponseEntity<Map<String, String>> createVnPayOnlinePayment(@RequestBody Map<String, Object> requestData,
        ...
        try {
            ...
            // Set idKhachHang if provided (for logged-in customers)
            if (orderMap.get("idKhachHang") != null) {
                onlineOrder.setIdKhachHang(((Number) orderMap.get("idKhachHang")).intValue());
            } else {
                onlineOrder.setIdKhachHang(null); // TH1: null nếu chưa đăng nhập
            }
    ...
    // Map to store pending order update requests
    private final Map<String, Map<String, Object>> pendingOrderUpdates = new HashMap<>();

    /**
     * API: Tạo VNPay payment URL cho cập nhật đơn hàng
     * Lưu order update request để dùng sau khi thanh toán thành công
     */
    @PostMapping("/vnpay/create-with-order-update")
    public ResponseEntity<?> createVnPayPaymentForOrderUpdate(@RequestBody Map<String, Object> requestData,
                                                               @RequestHeader(value = "X-Forwarded-For", required = false) String xff,
                                                               @RequestHeader(value = "X-Real-IP", required = false) String xri) {
        try {
            // Extract payment request
            Map<String, Object> paymentRequestMap = (Map<String, Object>) requestData.get("paymentRequest");
            CreatePaymentRequest paymentRequest = new CreatePaymentRequest();
            paymentRequest.setAmount(((Number) paymentRequestMap.get("amount")).longValue());
            paymentRequest.setOrderInfo((String) paymentRequestMap.get("orderInfo"));
            paymentRequest.setBankCode((String) paymentRequestMap.get("bankCode"));

            // Extract order update request
            Map<String, Object> orderUpdateRequest = (Map<String, Object>) requestData.get("orderUpdateRequest");
            String maHoaDon = (String) requestData.get("maHoaDon");

            if (maHoaDon == null || maHoaDon.isEmpty()) {
                return ResponseEntity.badRequest().body(Map.of(
                        "error", "Thiếu mã hóa đơn",
                        "message", "maHoaDon không được để trống"
                ));
            }

            // Store order update request temporarily
            String txnRef = java.util.UUID.randomUUID().toString().replace("-", "").substring(0, 12);
            Map<String, Object> updateData = new HashMap<>();
            updateData.put("maHoaDon", maHoaDon);
            updateData.put("orderUpdateRequest", orderUpdateRequest);
            pendingOrderUpdates.put(txnRef, updateData);

            System.out.println("✅ Stored order update request for: " + maHoaDon + ", txnRef: " + txnRef);
            ...
            e.printStackTrace();
            return ResponseEntity.badRequest().body(Map.of(
                    "error", "Lỗi tạo thanh toán VNPay cho cập nhật đơn hàng",
                    "message", e.getMessage(),
                    "details", e.getClass().getSimpleName()
            ));
        }
    ...
    public ResponseEntity<Map<String, Object>> handleVnPayReturn(
        ...
        try {
            ...
            if ("00".equals(responseCode) && "00".equals(transactionStatus)) {
                ...
                try {
                    // Check for order update request first
                    Map<String, Object> orderUpdateData = pendingOrderUpdates.get(txnRef);
                    if (orderUpdateData != null) {
                        String maHoaDon = (String) orderUpdateData.get("maHoaDon");
                        Map<String, Object> orderUpdateRequest = (Map<String, Object>) orderUpdateData.get("orderUpdateRequest");
                        
                        System.out.println("✅ Processing order update for: " + maHoaDon);
                        
                        // Update order with the stored request
                        ResponseEntity<?> updateResponse = hoaDonController.updateOrder(maHoaDon, orderUpdateRequest);
                        
                        if (updateResponse.getStatusCode().is2xxSuccessful()) {
                            Map<String, Object> updateBody = (Map<String, Object>) updateResponse.getBody();
                            if (updateBody != null && Boolean.TRUE.equals(updateBody.get("success"))) {
                                // Create additional fee payment record with trangThai = 1 (đã thanh toán)
                                try {
                                    Long vnpAmount = Long.parseLong(allParams.get("vnp_Amount"));
                                    Double priceDifference = vnpAmount / 100.0;
                                    hoaDonServiceImpl.createAdditionalFeePayment(
                                            hoaDonService.findByMaHoaDon(maHoaDon),
                                            priceDifference,
                                            "VNPay",
                                            "Thanh toán cập nhật đơn hàng - VNPay Transaction: " + allParams.get("vnp_TransactionNo"),
                                            1 // Đã thanh toán
                                    );
                                } catch (Exception feeError) {
                                    System.err.println("⚠️ Error creating additional fee: " + feeError.getMessage());
                                    feeError.printStackTrace();
                                }
                                
                                result.put("orderId", maHoaDon);
                                result.put("orderCode", maHoaDon);
                                result.put("isOrderUpdate", true);
                                
                                // Remove from pending order updates
                                pendingOrderUpdates.remove(txnRef);
                                
                                System.out.println("✅ Order update completed successfully for: " + maHoaDon);
                            } else {
                                result.put("orderUpdateError", "Cập nhật đơn hàng thất bại");
                            }
                        } else {
                            result.put("orderUpdateError", "Cập nhật đơn hàng thất bại");
                        }
