...
public class SanPhamController {
    ...
    @GetMapping("/{sanPhamId}/chi-tiet/all")
    public ResponseEntity<List<ChiTietSanPham>> getAllChiTietSanPhamBySanPhamId(@PathVariable Integer sanPhamId) {
        List<ChiTietSanPham> chiTietSanPhams = sanPhamService.getAllChiTietSanPhamBySanPhamId(sanPhamId);
        return ResponseEntity.ok(chiTietSanPhams);
    }
    ...
    public ResponseEntity<List<ChiTietSanPham>> getAllChiTietSanPham() {
        List<ChiTietSanPham> chiTietSanPhams = sanPhamService.getAllChiTietSanPham();
        return ResponseEntity.ok(chiTietSanPhams);
    }
    ...
    @PutMapping("/chi-tiet/{id}/status")
    public ResponseEntity<?> updateChiTietStatus(@PathVariable Integer id, @RequestBody Map<String, Integer> request) {
        try {
            Integer trangThai = request.get("trangThai");
            if (trangThai == null) {
                return ResponseEntity.badRequest().body(Map.of("error", "trangThai is required"));
            }
            sanPhamService.updateChiTietStatus(id, trangThai);
            return ResponseEntity.ok(Map.of("message", "Chi tiết sản phẩm status updated successfully"));
        } catch (RuntimeException e) {
            return ResponseEntity.status(HttpStatus.NOT_FOUND).body(Map.of(
                "error", "Không tìm thấy chi tiết sản phẩm",
                "message", e.getMessage()
            ));
        } catch (Exception e) {
            return ResponseEntity.status(HttpStatus.INTERNAL_SERVER_ERROR).body(Map.of(
